...
local Library = {
    ...
    MainFrameRef = nil,
    MinimizeBtnRef = nil,
    ScaleObj = nil, -- For UI Scaling
    Keybind = Enum.KeyCode.RightControl,
    IsVisible = true,
    Flags = {},
    ConfigFolder = "ModernUI_Configs",
    Version = "13.0 Ultimate (Overhaul)" -- UPDATED VERSION
}
...
Library.Themes = {
    ...
    PastelPurple = {
        ...
        Yellow = Color3.fromRGB(255, 255, 150),
    ...
    -- // NEW THEMES (V13 ADDITION)
    Midnight = {
        Background = Color3.fromRGB(10, 10, 15), Background2 = Color3.fromRGB(15, 15, 20), Sidebar = Color3.fromRGB(12, 12, 18), Section = Color3.fromRGB(20, 20, 25), Element = Color3.fromRGB(25, 25, 30),
        Text = Color3.fromRGB(255, 255, 255), SubText = Color3.fromRGB(150, 150, 170), Divider = Color3.fromRGB(40, 40, 50),
        Gradient1 = Color3.fromRGB(100, 100, 255), Gradient2 = Color3.fromRGB(50, 50, 200), Red = Color3.fromRGB(255, 50, 50), Green = Color3.fromRGB(50, 255, 100), Yellow = Color3.fromRGB(255, 200, 50), Shadow = Color3.fromRGB(0, 0, 5)
    },
    BloodMoon = {
        Background = Color3.fromRGB(20, 10, 10), Background2 = Color3.fromRGB(30, 15, 15), Sidebar = Color3.fromRGB(25, 12, 12), Section = Color3.fromRGB(35, 20, 20), Element = Color3.fromRGB(40, 25, 25),
        Text = Color3.fromRGB(255, 200, 200), SubText = Color3.fromRGB(180, 120, 120), Divider = Color3.fromRGB(60, 30, 30),
        Gradient1 = Color3.fromRGB(255, 0, 0), Gradient2 = Color3.fromRGB(150, 0, 0), Red = Color3.fromRGB(255, 50, 50), Green = Color3.fromRGB(50, 255, 100), Yellow = Color3.fromRGB(255, 200, 50), Shadow = Color3.fromRGB(10, 0, 0)
    },
    Oceanic = {
        Background = Color3.fromRGB(10, 20, 30), Background2 = Color3.fromRGB(15, 30, 45), Sidebar = Color3.fromRGB(12, 25, 35), Section = Color3.fromRGB(20, 40, 55), Element = Color3.fromRGB(25, 50, 65),
        Text = Color3.fromRGB(200, 240, 255), SubText = Color3.fromRGB(120, 160, 180), Divider = Color3.fromRGB(30, 60, 80),
        Gradient1 = Color3.fromRGB(0, 200, 255), Gradient2 = Color3.fromRGB(0, 100, 200), Red = Color3.fromRGB(255, 80, 80), Green = Color3.fromRGB(0, 255, 150), Yellow = Color3.fromRGB(255, 220, 50), Shadow = Color3.fromRGB(0, 10, 20)
    },
    Synthwave = {
        Background = Color3.fromRGB(20, 10, 30), Background2 = Color3.fromRGB(35, 15, 45), Sidebar = Color3.fromRGB(25, 12, 35), Section = Color3.fromRGB(40, 20, 50), Element = Color3.fromRGB(45, 25, 55),
        Text = Color3.fromRGB(255, 0, 255), SubText = Color3.fromRGB(0, 255, 255), Divider = Color3.fromRGB(60, 30, 80),
        Gradient1 = Color3.fromRGB(255, 0, 150), Gradient2 = Color3.fromRGB(0, 200, 255), Red = Color3.fromRGB(255, 50, 100), Green = Color3.fromRGB(50, 255, 200), Yellow = Color3.fromRGB(255, 255, 0), Shadow = Color3.fromRGB(20, 0, 30)
    }
}
...
function Library:SetTheme(ThemeName)
    if Library.Themes[ThemeName] then 
        ...
        Library:Notify({Title = "Theme", Content = "Switched to "..ThemeName, Type = "Info"})
    end
end

function Library:SetScale(Scale)
    if Library.ScaleObj then
        Library.ScaleObj.Scale = math.clamp(Scale, 0.5, 2)
    end
end
...
function Library:ToggleUI()
    ...
    if Library.MainFrameRef then
        Library.MainFrameRef.Visible = Library.IsVisible
    end
end
...
local function RegisterContainerFunctions(Container, PageInstance)
    ...
    function Funcs:AddSubSection(Title)
        local F = Create("Frame", {Parent = PageInstance, BackgroundTransparency = 1, Size = UDim2.new(1,0,0,25)})
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1,0,1,0), Position = UDim2.new(0,5,0,0), Text = Title, Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.SubText, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left})
        Create("Frame", {Parent = F, BackgroundColor3 = Library.Theme.Divider, Size = UDim2.new(1, -10 - (#Title*7), 0, 1), Position = UDim2.new(0, 10 + (#Title*7), 0.5, 0)})
        return F
    end
    ...
    function Funcs:Divide()
        ...
        local L = Create("Frame", {Parent = F, BackgroundColor3 = Library.Theme.Divider, Size = UDim2.new(1, -20, 0, 1), Position = UDim2.new(0, 10, 0.5, 0)})
        return F
    end

    function Funcs:Space(pixels)
        local F = Create("Frame", {Parent = PageInstance, BackgroundTransparency = 1, Size = UDim2.new(1,0,0, pixels or 20)})
        return F
    end
    ...
    function Funcs:AddLabel(Text, Color)
        local F = AddElementFrame(25)
        F.BackgroundTransparency = 1
        local L = Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 10, 0, 0), Text = Text, Font = Enum.Font.GothamBold, TextColor3 = Color or Library.Theme.Text, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left})
        local Obj = {}; AddAPI(Obj, F, {})
        function Obj:SetText(t) L.Text = t end
        function Obj:SetColor(c) L.TextColor3 = c end
        return Obj
    end
    
    function Funcs:AddStatusLabel(Text, Status) -- Green/Red dot
        local F = AddElementFrame(30)
        local Dot = Create("Frame", {Parent = F, BackgroundColor3 = Status and Library.Theme.Green or Library.Theme.Red, Size = UDim2.new(0,10,0,10), Position = UDim2.new(0,10,0.5,-5)}); Create("UICorner", {Parent = Dot, CornerRadius = UDim.new(1,0)})
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1, -30, 1, 0), Position = UDim2.new(0, 25, 0, 0), Text = Text, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left})
        local Obj = {}; AddAPI(Obj, F, {}); function Obj:SetStatus(s) Dot.BackgroundColor3 = s and Library.Theme.Green or Library.Theme.Red end; return Obj
    end
    
    function Funcs:AddConsole(Config) -- Scrollable log
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1, 0, 0, Config.Height or 150)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0, 6)})
        local H = Create("Frame", {Parent = F, BackgroundColor3 = Library.Theme.Section, Size = UDim2.new(1,0,0,25)}); Create("UICorner", {Parent = H, CornerRadius = UDim.new(0,6)})
        Create("TextLabel", {Parent = H, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,0), Size = UDim2.new(1,-20,1,0), Text = Config.Text or "Console", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left})
        
        local S = Create("ScrollingFrame", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,5,0,30), Size = UDim2.new(1,-10,1,-35), CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarThickness = 4})
        Create("UIListLayout", {Parent = S, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 2)})
        
        local Obj = {}; AddAPI(Obj, F, Config)
        function Obj:Log(Txt, Col)
            local L = Create("TextLabel", {Parent = S, BackgroundTransparency = 1, Size = UDim2.new(1,0,0,15), Font = Enum.Font.Code, Text = Txt, TextColor3 = Col or Library.Theme.SubText, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left})
            S.CanvasPosition = Vector2.new(0, S.AbsoluteCanvasSize.Y)
        end
        return Obj
    end

    function Funcs:AddTable(Config) -- Data Table
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0, 6)})
        local Rows = Create("Frame", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1,0,0,0), AutomaticSize = Enum.AutomaticSize.Y}); Create("UIListLayout", {Parent = Rows, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 1)})
        Create("UIPadding", {Parent = Rows, PaddingTop = UDim.new(0,5), PaddingBottom = UDim.new(0,5)})
        
        local Obj = {}; AddAPI(Obj, F, Config)
        function Obj:SetData(Data) -- Data = {{ "Row1Col1", "Row1Col2" }, ...}
            for _,v in pairs(Rows:GetChildren()) do if v:IsA("Frame") then v:Destroy() end end
            for _, rowData in pairs(Data) do
                local R = Create("Frame", {Parent = Rows, BackgroundColor3 = Library.Theme.Section, Size = UDim2.new(1, -10, 0, 25), Position = UDim2.new(0,5,0,0)}); Create("UICorner", {Parent = R, CornerRadius = UDim.new(0,4)})
                local layout = Create("UIListLayout", {Parent = R, FillDirection = Enum.FillDirection.Horizontal, SortOrder = Enum.SortOrder.LayoutOrder})
                for _, cell in pairs(rowData) do
                    Create("TextLabel", {Parent = R, BackgroundTransparency = 1, Size = UDim2.new(1/#rowData, 0, 1, 0), Text = tostring(cell), Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text, TextSize = 12})
                end
            end
        end
        if Config.Rows then Obj:SetData(Config.Rows) end
        return Obj
    end

    function Funcs:AddProgressBar(Config)
        local F = AddElementFrame(40)
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 5), Size = UDim2.new(1, -20, 0, 15), Text = Config.Text or "Progress", Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left})
        local Bar = Create("Frame", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0, 10, 0, 25), Size = UDim2.new(1, -20, 0, 6)}); Create("UICorner", {Parent = Bar, CornerRadius = UDim.new(1,0)})
        local Fill = Create("Frame", {Parent = Bar, BackgroundColor3 = Library.Theme.Gradient1, Size = UDim2.new(Config.Default or 0, 0, 1, 0)}); Create("UICorner", {Parent = Fill, CornerRadius = UDim.new(1,0)}); ApplyGradient(Fill)
        local Obj = {}; AddAPI(Obj, F, Config)
        function Obj:Set(val) TweenService:Create(Fill, TweenInfo.new(0.3), {Size = UDim2.new(math.clamp(val,0,1), 0, 1, 0)}):Play() end
        return Obj
    end

    function Funcs:AddImage(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1, 0, 0, Config.Height or 100)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0, 6)})
        local Img = Create("ImageLabel", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1, -10, 1, -10), Position = UDim2.new(0, 5, 0, 5), Image = "rbxassetid://"..Config.Id, ScaleType = Enum.ScaleType.Fit})
        local Obj = {}; AddAPI(Obj, F, Config); return Obj
    end

    function Funcs:AddImageButton(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1, 0, 0, Config.Height or 100)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0, 6)})
        local Img = Create("ImageButton", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1, -10, 1, -10), Position = UDim2.new(0, 5, 0, 5), Image = "rbxassetid://"..Config.Id, ScaleType = Enum.ScaleType.Fit})
        Img.MouseButton1Click:Connect(function() if Config.Callback then Config.Callback() end end)
        local Obj = {}; AddAPI(Obj, F, Config); return Obj
    end
    ...
    function Funcs:AddGitButton(Config) -- Fun extra
        local F = AddElementFrame(36)
        local Icon = Create("ImageLabel", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(0,24,0,24), Position = UDim2.new(0,10,0.5,-12), Image = "rbxassetid://3926305904", ImageColor3 = Library.Theme.Text}) -- Git logo fake
        local Lbl = Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0, 40, 0, 0), Size = UDim2.new(1, -40, 1, 0), Text = Config.Text or "Copy GitHub Link", Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left})
        ...
        Btn.MouseButton1Click:Connect(function()
            ...
            Library:Notify({Title = "GitHub", Content = "Copied to clipboard!", Type = "Success"})
        end)
        return F
    end
    ...
    -- DRAGGABLE LIST (FIXED: 0.67s Hold + Grow/Float + Numeric Input + Auto-Sort)
    function Funcs:AddDraggableList(Config)
        ...
        local function Refresh()
            ...
            for i, item in ipairs(Config.Items or {}) do
                ...
                -- Numeric Rank Box
                local RankBox = Create("TextBox", {Parent = IF, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0, 5, 0.5, -10), Size = UDim2.new(0, 24, 0, 20), Text = tostring(i), Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextSize = 11}); Create("UICorner", {Parent = RankBox, CornerRadius = UDim.new(0, 4)})
                
                Create("TextLabel", {Parent = IF, BackgroundTransparency = 1, Size = UDim2.new(1, -65, 1, 0), Position = UDim2.new(0, 35, 0, 0), Text = item, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left})
                ...
                -- Manual Rank Logic
                RankBox.FocusLost:Connect(function()
                    local n = tonumber(RankBox.Text)
                    if n then
                        n = math.clamp(math.floor(n), 1, #Buttons)
                        -- Reorder Logic
                        local Old = IF.LayoutOrder
                        if n ~= Old then
                             -- Shift others
                             for _, other in pairs(Buttons) do
                                 if other ~= IF then
                                    if Old < n then -- Moving down
                                        if other.LayoutOrder > Old and other.LayoutOrder <= n then
                                            other.LayoutOrder = other.LayoutOrder - 1
                                            other.TextBox.Text = tostring(other.LayoutOrder)
                                        end
                                    else -- Moving up
                                        if other.LayoutOrder >= n and other.LayoutOrder < Old then
                                            other.LayoutOrder = other.LayoutOrder + 1
                                            other.TextBox.Text = tostring(other.LayoutOrder)
                                        end
                                    end
                             ...
                             IF.LayoutOrder = n
                             RankBox.Text = tostring(n)
                             FireCallback()
                        end
                    else
                        RankBox.Text = tostring(IF.LayoutOrder)
                    end
                end)

                -- Drag Logic with 0.67s Hold
                local Dragging = false
                local HoldStart = 0
                local Holding = false
                        ...
                        task.spawn(function()
                            while Holding do
                                if tick() - HoldStart >= 0.67 then
                                    ...
                                    if PageInstance:IsA("ScrollingFrame") then PageInstance.ScrollingEnabled = false end
                                    break
                                end
                                task.wait(0.1)
                            end
                        ...
                        input.Changed:Connect(function() 
                            if input.UserInputState == Enum.UserInputState.End then
                                ...
                                if Dragging then
                                    ...
                                    -- Reset Visuals
                                    IF.ZIndex = 1
                                    TweenService:Create(IF, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 36), BackgroundColor3 = Library.Theme.Element}):Play()
                                    if PageInstance:IsA("ScrollingFrame") then PageInstance.ScrollingEnabled = true end
                                    FireCallback()
                                    -- Refresh numbers
                                    for _, btn in pairs(Buttons) do btn.TextBox.Text = tostring(btn.LayoutOrder) end
                                end
                            end 
                        end)
                    end
                        ...
                        for _, other in pairs(Buttons) do
                            if other ~= IF then
                                ...
                                if CY > OY and CY < (OY + other.AbsoluteSize.Y) then
                                    ...
                                    -- Update visual numbers immediately for feedback
                                    RankBox.Text = tostring(IF.LayoutOrder)
                                    other.TextBox.Text = tostring(other.LayoutOrder)
                                end
                            end
    ...
    function Funcs:AddColorPicker(Config) -- Full V12 ColorPicker with Rainbow
        local F = AddElementFrame(130)
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,5), Size = UDim2.new(1,-50,0,20), Text = Config.Text, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text, TextXAlignment = Enum.TextXAlignment.Left})
        local P = Create("Frame", {Parent = F, Position = UDim2.new(1,-40,0,5), Size = UDim2.new(0,30,0,20), BackgroundColor3 = Config.Default or Color3.new(1,1,1)}); Create("UICorner", {Parent = P, CornerRadius = UDim.new(0,4)})
        
        -- RGB Inputs (Simplified visual for now)
        local R, G, B = 255, 255, 255
        local function Upd() P.BackgroundColor3 = Color3.fromRGB(R,G,B); if Config.Callback then Config.Callback(P.BackgroundColor3) end end
        
        local BoxR = Create("TextBox", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0,10,0,30), Size = UDim2.new(0.3,-10,0,20), Text = "R: 255", TextColor3 = Library.Theme.SubText, Font = Enum.Font.Gotham})
        local BoxG = Create("TextBox", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0.33,10,0,30), Size = UDim2.new(0.3,-10,0,20), Text = "G: 255", TextColor3 = Library.Theme.SubText, Font = Enum.Font.Gotham})
        local BoxB = Create("TextBox", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0.66,10,0,30), Size = UDim2.new(0.3,-10,0,20), Text = "B: 255", TextColor3 = Library.Theme.SubText, Font = Enum.Font.Gotham})
        
        local function Parse(t) return tonumber(t:match("%d+")) or 0 end
        BoxR.FocusLost:Connect(function() R = math.clamp(Parse(BoxR.Text),0,255); BoxR.Text="R: "..R; Upd() end)
        BoxG.FocusLost:Connect(function() G = math.clamp(Parse(BoxG.Text),0,255); BoxG.Text="G: "..G; Upd() end)
        BoxB.FocusLost:Connect(function() B = math.clamp(Parse(BoxB.Text),0,255); BoxB.Text="B: "..B; Upd() end)
        
        -- Rainbow Toggle inside Picker
        local RB = Create("TextButton", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0,10,0,60), Size = UDim2.new(1,-20,0,20), Text = "Rainbow Mode: OFF", Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText})
        local RBOn = false
        RB.MouseButton1Click:Connect(function() 
            RBOn = not RBOn; RB.Text = "Rainbow Mode: "..(RBOn and "ON" or "OFF"); RB.TextColor3 = RBOn and Library.Theme.Gradient1 or Library.Theme.SubText
            if RBOn then task.spawn(function() while RBOn do local h=tick()%5/5; local c=Color3.fromHSV(h,1,1); P.BackgroundColor3=c; if Config.Callback then Config.Callback(c) end; task.wait() end end) end
        end)
    ...
    -- =============================================================================================================
    -- // START OF V13 ULTIMATE EXPANSION PACK (100+ IMPROVEMENTS)
    -- =============================================================================================================
    
    -- 1. SEARCH LIST (Filterable list)
    function Funcs:AddSearchList(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1, 0, 0, Config.Height or 150)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0, 6)})
        local SBar = Create("TextBox", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0,10,0,10), Size = UDim2.new(1,-20,0,25), Text = "", PlaceholderText = "Search list...", Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text}); Create("UICorner", {Parent = SBar, CornerRadius = UDim.new(0,4)})
        local Scroll = Create("ScrollingFrame", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,5,0,40), Size = UDim2.new(1,-10,1,-45), CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarThickness = 4}); Create("UIListLayout", {Parent = Scroll, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0,2)})
        
        local function Refresh()
            for _,v in pairs(Scroll:GetChildren()) do if v:IsA("TextButton") then v:Destroy() end end
            for _, item in pairs(Config.Items or {}) do
                if SBar.Text == "" or item:lower():find(SBar.Text:lower()) then
                    local B = Create("TextButton", {Parent = Scroll, BackgroundColor3 = Library.Theme.Section, Size = UDim2.new(1,0,0,25), Text = item, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText}); Create("UICorner", {Parent = B, CornerRadius = UDim.new(0,4)})
                    B.MouseButton1Click:Connect(function() if Config.Callback then Config.Callback(item) end end)
                end
            end
        end
        SBar:GetPropertyChangedSignal("Text"):Connect(Refresh); Refresh()
        local Obj = {}; AddAPI(Obj, F, Config); return Obj
    end

    -- 2. RICH TEXT LABEL
    function Funcs:AddRichLabel(Text)
        local F = AddElementFrame(25); F.BackgroundTransparency = 1
        local L = Create("TextLabel", {Parent = F, RichText = true, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 10, 0, 0), Text = Text, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left})
        return F
    end

    -- 3. KEYPAD (Secure PIN Entry)
    function Funcs:AddKeypad(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1,0,0,180)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)})
        local Disp = Create("TextLabel", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(0,10,0,10), Size = UDim2.new(1,-20,0,30), Text = "", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Gradient1, TextSize = 18}); Create("UICorner", {Parent = Disp, CornerRadius = UDim.new(0,4)})
        local Grid = Create("Frame", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,30,0,50), Size = UDim2.new(1,-60,1,-60)})
        local GL = Create("UIGridLayout", {Parent = Grid, CellSize = UDim2.new(0,35,0,35), CellPadding = UDim2.new(0,10,0,10), HorizontalAlignment = Enum.HorizontalAlignment.Center})
        for i=1,9 do 
            local B = Create("TextButton", {Parent = Grid, BackgroundColor3 = Library.Theme.Section, Text = tostring(i), Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text}); Create("UICorner", {Parent = B, CornerRadius = UDim.new(0,4)})
            B.MouseButton1Click:Connect(function() if #Disp.Text < 4 then Disp.Text = Disp.Text..i end end)
        end
        local Clear = Create("TextButton", {Parent = Grid, BackgroundColor3 = Library.Theme.Red, Text = "C", Font = Enum.Font.GothamBold, TextColor3 = Color3.new(1,1,1)}); Create("UICorner", {Parent = Clear, CornerRadius = UDim.new(0,4)})
        Clear.MouseButton1Click:Connect(function() Disp.Text = "" end)
        local Zero = Create("TextButton", {Parent = Grid, BackgroundColor3 = Library.Theme.Section, Text = "0", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text}); Create("UICorner", {Parent = Zero, CornerRadius = UDim.new(0,4)})
        Zero.MouseButton1Click:Connect(function() if #Disp.Text < 4 then Disp.Text = Disp.Text.."0" end end)
        local Enter = Create("TextButton", {Parent = Grid, BackgroundColor3 = Library.Theme.Green, Text = "OK", Font = Enum.Font.GothamBold, TextColor3 = Color3.new(1,1,1)}); Create("UICorner", {Parent = Enter, CornerRadius = UDim.new(0,4)})
        Enter.MouseButton1Click:Connect(function() if Config.Callback then Config.Callback(Disp.Text) end; Disp.Text = "" end)
        return F
    end

    -- 4. VIDEO PLAYER (Image Sequence Simulation placeholder or actual VideoFrame if enabled)
    function Funcs:AddVideo(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Color3.new(0,0,0), Size = UDim2.new(1,0,0,150)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)})
        local Vid = Create("VideoFrame", {Parent = F, Size = UDim2.new(1,0,1,0), Video = "rbxassetid://"..(Config.Id or 5608327482), Looped = true, Playing = true, Volume = 0})
        if not pcall(function() Vid.Playing = true end) then -- Fallback if VideoFrame not supported/permissioned
            Vid:Destroy(); Create("TextLabel", {Parent = F, Size = UDim2.new(1,0,1,0), Text = "VideoFrame Not Permitted", TextColor3 = Color3.new(1,1,1), BackgroundTransparency = 1})
        end
        return F
    end

    -- 5. CODE BLOCK (Display Lua code)
    function Funcs:AddCodeBlock(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Color3.fromRGB(20,20,20), Size = UDim2.new(1,0,0,100)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)})
        local S = Create("ScrollingFrame", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1,-10,1,-10), Position = UDim2.new(0,5,0,5), CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.XY})
        Create("TextLabel", {Parent = S, AutomaticSize=Enum.AutomaticSize.XY, Text = Config.Code or "print('Hello')", Font = Enum.Font.Code, TextColor3 = Color3.fromRGB(100,255,100), TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top, BackgroundTransparency = 1})
        return F
    end

    -- 6. SYSTEM STATS (FPS/Ping)
    function Funcs:AddSystemStats()
        local F = AddElementFrame(60)
        local FPSLabel = Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,5), Size = UDim2.new(0.5,-10,0,25), Text = "FPS: ...", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text, TextXAlignment=Enum.TextXAlignment.Left})
        local PingLabel = Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0.5,0,0,5), Size = UDim2.new(0.5,-10,0,25), Text = "Ping: ...", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text, TextXAlignment=Enum.TextXAlignment.Left})
        local TimeLabel = Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,30), Size = UDim2.new(1,-20,0,25), Text = "Time: ...", Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextXAlignment=Enum.TextXAlignment.Left})
        ...
        task.spawn(function()
            while F.Parent do
                ...
                TimeLabel.Text = "Server Time: " .. os.date("%X")
                task.wait(1)
            end
        end)
        return F
    ...
    -- 7. DISCORD INVITE (Stylized)
    function Funcs:AddDiscordInvite(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Color3.fromRGB(88, 101, 242), Size = UDim2.new(1,0,0,40)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)})
        Create("ImageLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0.5,-12), Size = UDim2.new(0,24,0,24), Image = "rbxassetid://9505378375"}) -- Discord logo
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,40,0,0), Size = UDim2.new(1,-120,1,0), Text = Config.Title or "Join our Discord", Font = Enum.Font.GothamBold, TextColor3 = Color3.new(1,1,1), TextXAlignment = Enum.TextXAlignment.Left})
        local Join = Create("TextButton", {Parent = F, BackgroundColor3 = Color3.new(1,1,1), Position = UDim2.new(1,-70,0.5,-12), Size = UDim2.new(0,60,0,24), Text = "Join", Font = Enum.Font.GothamBold, TextColor3 = Color3.fromRGB(88,101,242)}); Create("UICorner", {Parent = Join, CornerRadius = UDim.new(0,4)})
        Join.MouseButton1Click:Connect(function() 
            if setclipboard then setclipboard(Config.Invite) end
            Library:Notify({Title = "Discord", Content = "Invite copied to clipboard!", Type = "Success"})
        end)
        return F
    end

    -- 8. MINI PLAYER (Fake music player)
    function Funcs:AddMusicPlayer(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1,0,0,90)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)})
        local Cover = Create("Frame", {Parent = F, BackgroundColor3 = Library.Theme.Gradient1, Position = UDim2.new(0,10,0,10), Size = UDim2.new(0,70,0,70)}); Create("UICorner", {Parent = Cover, CornerRadius = UDim.new(0,4)})
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,90,0,15), Size = UDim2.new(1,-100,0,20), Text = Config.Title or "Song Title", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text, TextXAlignment = Enum.TextXAlignment.Left})
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,90,0,35), Size = UDim2.new(1,-100,0,15), Text = Config.Artist or "Artist Name", Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextXAlignment = Enum.TextXAlignment.Left})
        
        local PlayBtn = Create("TextButton", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,90,0,55), Size = UDim2.new(0,20,0,20), Text = "▶", TextColor3 = Library.Theme.Text, TextSize = 18})
        local Playing = false
        PlayBtn.MouseButton1Click:Connect(function() Playing = not Playing; PlayBtn.Text = Playing and "II" or "▶" end)
        return F
    end

    -- 9. PAINT CANVAS (Fun interactive element)
    function Funcs:AddPaintCanvas(Config)
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Color3.new(1,1,1), Size = UDim2.new(1,0,0,150)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)})
        local Clear = Create("TextButton", {Parent = F, BackgroundColor3 = Library.Theme.Red, Size = UDim2.new(0,50,0,20), Position = UDim2.new(1,-55,0,5), Text = "Clear", TextColor3 = Color3.new(1,1,1)}); Create("UICorner", {Parent = Clear, CornerRadius = UDim.new(0,4)})
        local Canvas = Create("Frame", {Parent = F, BackgroundTransparency = 1, Size = UDim2.new(1,0,1,0), ClipsDescendants = true})
        
        local Drawing = false
        Canvas.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then Drawing = true end end)
        Canvas.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then Drawing = false end end)
        Canvas.InputChanged:Connect(function(i)
            if Drawing and i.UserInputType == Enum.UserInputType.MouseMovement then
                local Dot = Create("Frame", {Parent = Canvas, BackgroundColor3 = Color3.new(0,0,0), Size = UDim2.new(0,4,0,4), Position = UDim2.new(0, i.Position.X - F.AbsolutePosition.X, 0, i.Position.Y - F.AbsolutePosition.Y)}); Create("UICorner", {Parent = Dot, CornerRadius = UDim.new(1,0)})
            end
        end)
        Clear.MouseButton1Click:Connect(function() Canvas:ClearAllChildren() end)
        return F
    end

    -- 10. MINI RADAR (Shows blips for other players)
    function Funcs:AddMiniMap()
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Color3.fromRGB(0,20,0), Size = UDim2.new(1,0,0,200), ClipsDescendants = true}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,100)}); Create("UIStroke", {Parent = F, Color = Library.Theme.Green, Thickness = 2})
        local Center = Create("Frame", {Parent = F, BackgroundColor3 = Color3.new(1,1,1), Size = UDim2.new(0,6,0,6), Position = UDim2.new(0.5,-3,0.5,-3)}); Create("UICorner", {Parent = Center, CornerRadius = UDim.new(1,0)})
        
        task.spawn(function()
            while F.Parent do
                for _,v in pairs(F:GetChildren()) do if v.Name == "Blip" then v:Destroy() end end
                ...
                for _, p in pairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        local Rel = Pos - MyPos
                        local Scale = 2 -- Zoom
                        local X, Z = Rel.X / Scale, Rel.Z / Scale
                        if math.abs(X) < 90 and math.abs(Z) < 90 then
                            local Blip = Create("Frame", {Name="Blip", Parent = F, BackgroundColor3 = Library.Theme.Red, Size = UDim2.new(0,4,0,4), Position = UDim2.new(0.5, X, 0.5, Z)}); Create("UICorner", {Parent = Blip, CornerRadius = UDim.new(1,0)})
                        end
                    end
                end
                task.wait(0.1)
    ...
    -- 11. CONFETTI (Fun Effect)
    function Funcs:TriggerConfetti()
        local Emitter = Create("Frame", {Parent = Library.ScreenGuiRef, BackgroundTransparency = 1, Position = UDim2.new(0.5,0,0.5,0)})
        for i=1, 50 do
            task.spawn(function()
                local P = Create("Frame", {Parent = Emitter, Size = UDim2.new(0,8,0,8), BackgroundColor3 = Color3.fromHSV(math.random(),1,1), Position = UDim2.new(0,0,0,0)})
                local Dest = UDim2.new(0, math.random(-300,300), 0, math.random(-300,300))
                TweenService:Create(P, TweenInfo.new(1), {Position = Dest, Rotation = math.random(0,360), BackgroundTransparency = 1}):Play()
                task.wait(1)
                P:Destroy()
            end)
        end
        task.delay(1.5, function() Emitter:Destroy() end)
    end
    
    -- 12. TOAST NOTIFICATION (Alternative Style)
    function Funcs:AddToastButton(Config)
        local Btn = Funcs:AddButton({Text = Config.Text})
        Btn.GetFrame().TextButton.MouseButton1Click:Connect(function()
            local T = Create("Frame", {Parent = Library.ScreenGuiRef, BackgroundColor3 = Color3.fromRGB(40,40,40), Size = UDim2.new(0, 200, 0, 40), Position = UDim2.new(0.5, -100, 0.8, 0)}); Create("UICorner", {Parent = T, CornerRadius = UDim.new(0,20)})
            Create("TextLabel", {Parent = T, BackgroundTransparency = 1, Size = UDim2.new(1,0,1,0), Text = Config.Message or "Action Complete", TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold})
            TweenService:Create(T, TweenInfo.new(0.5), {Position = UDim2.new(0.5, -100, 0.7, 0)}):Play()
            task.delay(2, function() T:Destroy() end)
        end)
        return Btn
    end

    -- 13. WEATHER WIDGET (Fake)
    function Funcs:AddWeatherWidget()
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Gradient1, Size = UDim2.new(1,0,0,80)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)}); ApplyGradient(F)
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,10), Size = UDim2.new(0,100,0,30), Text = "24°C", TextSize = 30, Font = Enum.Font.GothamBold, TextColor3 = Color3.new(1,1,1), TextXAlignment = Enum.TextXAlignment.Left})
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,45), Size = UDim2.new(0,100,0,20), Text = "Sunny", TextSize = 14, Font = Enum.Font.Gotham, TextColor3 = Color3.new(1,1,1), TextXAlignment = Enum.TextXAlignment.Left})
        Create("ImageLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(1,-70,0,10), Size = UDim2.new(0,60,0,60), Image = "rbxassetid://13851502421"}) -- Sun Icon
        return F
    end

    -- 14. PROFILE CARD (Detailed)
    function Funcs:AddProfileCard(UserId)
        local Id = UserId or LocalPlayer.UserId
        local F = Create("Frame", {Parent = PageInstance, BackgroundColor3 = Library.Theme.Element, Size = UDim2.new(1,0,0,100)}); Create("UICorner", {Parent = F, CornerRadius = UDim.new(0,6)})
        local PFP = Create("ImageLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,10), Size = UDim2.new(0,80,0,80), Image = Players:GetUserThumbnailAsync(Id, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100)}); Create("UICorner", {Parent = PFP, CornerRadius = UDim.new(1,0)})
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,100,0,20), Size = UDim2.new(1,-110,0,20), Text = Players:GetNameFromUserIdAsync(Id), Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text, TextSize = 18, TextXAlignment = Enum.TextXAlignment.Left})
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,100,0,45), Size = UDim2.new(1,-110,0,15), Text = "ID: "..Id, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left})
        return F
    end

    -- 15. STEPPER (Increment/Decrement)
    function Funcs:AddStepper(Config)
        local F = AddElementFrame(36)
        local Val = Config.Default or 0
        Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(0,10,0,0), Size = UDim2.new(1,-100,1,0), Text = Config.Text, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.Text, TextXAlignment = Enum.TextXAlignment.Left})
        local Minus = Create("TextButton", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(1,-90,0.5,-10), Size = UDim2.new(0,20,0,20), Text = "-", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text}); Create("UICorner", {Parent = Minus, CornerRadius = UDim.new(0,4)})
        local Disp = Create("TextLabel", {Parent = F, BackgroundTransparency = 1, Position = UDim2.new(1,-65,0,0), Size = UDim2.new(0,30,1,0), Text = tostring(Val), Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Gradient1})
        local Plus = Create("TextButton", {Parent = F, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(1,-30,0.5,-10), Size = UDim2.new(0,20,0,20), Text = "+", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.Text}); Create("UICorner", {Parent = Plus, CornerRadius = UDim.new(0,4)})
        
        Minus.MouseButton1Click:Connect(function() Val = Val - (Config.Step or 1); Disp.Text = tostring(Val); if Config.Callback then Config.Callback(Val) end end)
        Plus.MouseButton1Click:Connect(function() Val = Val + (Config.Step or 1); Disp.Text = tostring(Val); if Config.Callback then Config.Callback(Val) end end)
        return F
    end
...
function Library:CreateWindow(Config)
    if Library.ScreenGuiRef then Library.ScreenGuiRef:Destroy() end
    local Gui = Create("ScreenGui", {Name = "ModernUI_V12", Parent = CoreGui, ZIndexBehavior = Enum.ZIndexBehavior.Sibling})
    ...
    -- SHADOW V12
    local Shadow = Create("ImageLabel", {Parent = Main, BackgroundTransparency = 1, Position = UDim2.new(0,-15,0,-15), Size = UDim2.new(1,30,1,30), Image = "rbxassetid://5554236805", ScaleType = Enum.ScaleType.Slice, SliceCenter = Rect.new(23,23,277,277), ImageColor3 = Library.Theme.Shadow, ZIndex = -1})
    
    Create("UIStroke", {Parent = Main, Color = Library.Theme.Divider, Thickness = 1})
    ...
    Create("TextLabel", {Parent = Profile, BackgroundTransparency = 1, Position = UDim2.new(0, 50, 0, 20), Size = UDim2.new(0, 80, 0, 15), Font = Enum.Font.Gotham, Text = Library.Version, TextColor3 = Library.Theme.Gradient1, TextSize = 10, TextXAlignment = Enum.TextXAlignment.Left})
    ...
    -- BUILT-IN SETTINGS PAGE (ENCHANTED)
    ...
    SettingsHandler:AddChipSet({Text = "Themes", Items = {"Dark", "PastelPurple", "Sky", "Garden", "Midnight", "BloodMoon", "Oceanic", "Synthwave"}, Callback = function(v) Library:SetTheme(v) end})
    SettingsHandler:AddSlider({Text = "UI Scale", Min = 50, Max = 150, Default = 100, Callback = function(v) Library:SetScale(v/100) end})
