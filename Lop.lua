...
local function AddTooltip(Element, Text)
    ...
    Element.MouseEnter:Connect(function()
        ...
        TooltipFrame = Create("Frame", {
            Parent = Gui, BackgroundColor3 = Library.Theme.Background2, Size = UDim2.new(0,0,0,24), AutomaticSize = Enum.AutomaticSize.X,
            ZIndex = 100, BorderSizePixel = 0
        })
        Create("UICorner", {Parent = TooltipFrame, CornerRadius = UDim.new(0, 4)})
        Create("UIStroke", {Parent = TooltipFrame, Color = Library.Theme.Divider, Thickness = 1})
        Create("TextLabel", {
            Parent = TooltipFrame, Text = "  "..Text.."  ", TextColor3 = Library.Theme.Text, Font = Enum.Font.Gotham, TextSize = 11,
...
local function RegisterContainerFunctions(Container, PageInstance)
    ...
    function Funcs:AddButton(Config)
        ...
        Btn.MouseButton1Click:Connect(function()
            ...
            if Config.Callback then
                ...
                if not success then Library:Notify({Title="Error", Content=err, Type="Error"}) end
            end
        end)
    end
    ...
    function Funcs:AddSection(Config)
        ...
        local Title = Create("TextLabel", {Parent = Header, Text = Config.Text, Font = Enum.Font.GothamBold, TextSize = 13, TextColor3 = Library.Theme.Text, BackgroundTransparency = 1, Size = UDim2.new(1,-30,1,0), Position = UDim2.new(0,10,0,0), TextXAlignment = Enum.TextXAlignment.Left})
        ...
        local ContainerFrame = Create("Frame", {Parent = Main, BackgroundTransparency = 1, Size = UDim2.new(1,0,0,0), AutomaticSize = Enum.AutomaticSize.Y, Visible = IsOpen})
        ...
        Header.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(Line, TweenInfo.new(0.3), {Size = IsOpen and UDim2.new(1,-20,0,2) or UDim2.new(0,0,0,2)}):Play()
        end)
    ...
    function Funcs:AddToggle(Config)
        ...
        if Variant == "Lock" then
            Create("ImageLabel", {Parent = TFrame, Image = "rbxassetid://6031094636", Size = UDim2.new(0,20,0,20), Position = UDim2.new(1,-30,0.5,-10), BackgroundTransparency = 1, ImageColor3 = Library.Theme.SubText})
    ...
    function Funcs:AddSlider(Config)
        ...
        ValBox.FocusLost:Connect(function()
            ...
            if n then n = math.clamp(n, Min, Max); ValBox.Text = tostring(n); Fill.Size = UDim2.new((n-Min)/(Max-Min), 0, 1, 0); if Config.Callback then Config.Callback(n) end else ValBox.Text = tostring(Def) end
        end)
    ...
    function Funcs:AddKeybind(Config)
        ...
        local BindBtn = Create("TextButton", {Parent = KFrame, BackgroundColor3 = Library.Theme.Section, Position = UDim2.new(1, -100, 0.5, -12), Size = UDim2.new(0, 90, 0, 24), Text = Config.Default and Config.Default.Name or "None", Font = Enum.Font.GothamBold, TextColor3 = Library.Theme.SubText, TextSize = 12})
        Create("UICorner", {Parent = BindBtn, CornerRadius = UDim.new(0, 4)})
        
        local Listening = false
        BindBtn.MouseButton1Click:Connect(function()
            Listening = true; BindBtn.Text = "..."; BindBtn.TextColor3 = Library.Theme.Gradient1
        end)
    ...
    function Funcs:AddProgressBar(Config)
        ...
        local Handler = {}
        function Handler:Update(Val) -- 0 to 100
                ...
                if txt == "" or item:lower():find(txt:lower()) then
                    ...
                    B.MouseButton1Click:Connect(function()
                        if IsMulti then
                            ...
                            Refresh(SearchBar.Text) -- Refresh color state
                        else
                            IsOpen = false; TweenService:Create(DFrame, TweenInfo.new(0.3), {Size = UDim2.new(1, 0, 0, 40)}):Play(); TweenService:Create(Arrow, TweenInfo.new(0.3), {Rotation = 0}):Play(); Label.Text = Config.Text..": "..item; if Config.Callback then Config.Callback(item) end
                        end
                    end)
                end
            end
    ...
    function Funcs:AddColorPicker(Config)
        ...
        local function AddInput(Name, Pos, Get, Set)
            ...
            Box.FocusLost:Connect(function() local n = tonumber(Box.Text); if n then Set(math.clamp(n,0,255)) end; local c = Preview.BackgroundColor3; Preview.BackgroundColor3 = Color3.fromRGB(c.R*255, c.G*255, c.B*255); Config.Callback(Preview.BackgroundColor3) end); return Box
        end
        ...
        -- Rainbow Toggle in Picker
        local RainbowBtn = Create("TextButton", {Parent = PickerFrame, Position = UDim2.new(0, 10, 0, 100), Size = UDim2.new(1, -20, 0, 24), BackgroundColor3 = Library.Theme.Section, Text = "Rainbow Mode", TextColor3 = Library.Theme.SubText, Font = Enum.Font.GothamBold, TextSize = 11}); Create("UICorner", {Parent = RainbowBtn, CornerRadius = UDim.new(0,4)})
        local RB = false; local Loop
    ...
    function Funcs:AddParagraph(Config)
        ...
        if Config.Image then
            local Img = Create("ImageLabel", {Parent = PFrame, Image = Config.Image, BackgroundTransparency = 1, Size = UDim2.new(0, 200, 0, 120), Position = UDim2.new(0.5,-100,0,30)})
            Create("UICorner", {Parent = Img, CornerRadius = UDim.new(0,6)})
            return -- Simple Image Paragraph
        end
    ...
    function Funcs:AddLabel(Config)
        ...
        local Func = {}
        function Func:Set(Txt) Lbl.Text = Txt end
        return Func
    end
...
function Library:KeySystem(Config)
    local KWin = Create("ScreenGui", {Parent = CoreGui})
    local Main = Create("Frame", {Parent = KWin, BackgroundColor3 = Library.Theme.Background, Size = UDim2.new(0, 400, 0, 250), Position = UDim2.new(0.5, -200, 0.5, -125)}); Create("UICorner", {Parent = Main, CornerRadius = UDim.new(0, 8)}); Create("UIStroke", {Parent = Main, Color = Library.Theme.Gradient1, Thickness = 1})
    Create("TextLabel", {Parent = Main, Text = Config.Title or "Key System", Font = Enum.Font.GothamBold, TextSize = 24, TextColor3 = Library.Theme.Text, Size = UDim2.new(1, 0, 0, 50), BackgroundTransparency = 1})
    local Box = Create("TextBox", {Parent = Main, BackgroundColor3 = Library.Theme.Section, Size = UDim2.new(0.8, 0, 0, 40), Position = UDim2.new(0.1, 0, 0.4, 0), Text = "", PlaceholderText = "Enter Key...", TextColor3 = Library.Theme.Text, Font = Enum.Font.Gotham}); Create("UICorner", {Parent = Box, CornerRadius = UDim.new(0, 6)})
    local Btn = Create("TextButton", {Parent = Main, BackgroundColor3 = Library.Theme.Gradient1, Size = UDim2.new(0.4, 0, 0, 35), Position = UDim2.new(0.3, 0, 0.7, 0), Text = "Submit", TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold}); Create("UICorner", {Parent = Btn, CornerRadius = UDim.new(0, 6)})
    Btn.MouseButton1Click:Connect(function() if Box.Text == Config.Key then KWin:Destroy(); if Config.Callback then Config.Callback() end else Box.Text = "Incorrect Key!"; task.wait(1); Box.Text = "" end end)
end

--// MAIN WINDOW
function Library:CreateWindow(Config)
    local Window = {Tabs = {}, IsVisual = true}
    local ScreenGui = Create("ScreenGui", {Name = "LopUI_V7_GodMode", Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or CoreGui, ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling})
    Library:InitNotifications(ScreenGui)

    -- ACRYLIC EFFECT (Simple Darken for now)
    local Blur = Instance.new("BlurEffect", Lighting); Blur.Size = 0; Blur.Name = "LopUI_Blur"
    ...
    function Window:SetVisual(bool) 
        Library.IsVisible = bool
        MainFrame.Visible = bool
        TweenService:Create(Blur, TweenInfo.new(0.3), {Size = bool and 20 or 0}):Play()
    end
    ...
    SettingsHandler:AddSlider({Text = "UI Scale", Min = 70, Max = 150, Default = 100, Callback = function(v) MainFrame.UIScale.Scale = v/100 end})
    Create("UIScale", {Parent = MainFrame, Scale = 1}) -- Ensure UIScale exists
    ...
    SettingsHandler:AddButton({Text = "Server Hop", Callback = function() 
        Library:Notify({Title="Hopping", Content="Finding new server..."})
        ...
        for _,v in pairs(x.data) do if v.playing < v.maxPlayers then TeleportService:TeleportToPlaceInstance(game.PlaceId, v.id, LocalPlayer) end end
    end})
    ...
    SettingsHandler:AddButton({Text = "Unload UI", Variant="Danger", Callback = function() ScreenGui:Destroy(); Blur:Destroy() end})
