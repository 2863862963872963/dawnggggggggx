...
--[[
    ...
    ─────────────────
    Yerba:SetDPI(scale)
        Sets the UI scale for every open window.
        scale  – a number. 1 = 100% (default), 0.8 = 80%, 1.5 = 150%, etc.
        Accepts a "dpi" shorthand too: pass 96 (= 1×), 144 (= 1.5×), 72 (= 0.75×).
        Any value ≥ 4 is treated as a raw DPI number and converted (value / 96).
        Clamps between 0.5 and 3.0.

    Yerba:GetDPI()
        Returns the current scale factor (e.g. 1, 0.8, 1.25).

    Window:SetDPI(scale)
        Same as above but only affects a single window.
...
function Yerba:SetDPI(value)
	...
	Yerba.DPI = scale
	for _, win in pairs(Yerba.Windows) do
		if win._UIScale then
			win._dpi = scale
			ApplyScale(win._UIScale, scale)
		end
	end
end
...
--[[
	Yerba:SetTheme(preset)
	  preset – string name OR a custom table with any Theme keys you want to override.

	Built-in presets: "dark" (default), "green", "blue", "red", "purple"

	Example – built-in:
	  Yerba:SetTheme("green")

	Example – custom override (only changes the keys you provide):
	  Yerba:SetTheme({ Accent = Color3.fromRGB(180, 60, 200), Background = Color3.fromRGB(10, 5, 15) })
--]]
local ThemePresets = {
	dark = {
	...
	green = {
		...
		Accent     = Color3.fromRGB(50, 140, 80),
		AccentDark = Color3.fromRGB(25, 65, 38),
		AccentMid  = Color3.fromRGB(50, 130, 75),
		AccentBlue = Color3.fromRGB(20, 130, 60),
		Blue       = Color3.fromRGB(55, 180, 90),
		Enabled    = Color3.fromRGB(55, 180, 90),
	},
	blue = {
		Background = Color3.fromRGB(8, 10, 18),
		Accent     = Color3.fromRGB(50, 90, 200),
		AccentDark = Color3.fromRGB(25, 40, 100),
		AccentMid  = Color3.fromRGB(50, 85, 190),
		AccentBlue = Color3.fromRGB(30, 80, 210),
		Blue       = Color3.fromRGB(70, 140, 255),
		Enabled    = Color3.fromRGB(70, 140, 255),
	},
	red = {
		Background = Color3.fromRGB(15, 8, 8),
		Accent     = Color3.fromRGB(180, 55, 55),
		AccentDark = Color3.fromRGB(90, 25, 25),
		AccentMid  = Color3.fromRGB(170, 50, 50),
		AccentBlue = Color3.fromRGB(160, 40, 40),
		Blue       = Color3.fromRGB(220, 70, 70),
		Enabled    = Color3.fromRGB(220, 70, 70),
	},
	purple = {
		Background = Color3.fromRGB(10, 8, 16),
		Accent     = Color3.fromRGB(130, 70, 200),
		AccentDark = Color3.fromRGB(60, 30, 100),
		AccentMid  = Color3.fromRGB(120, 65, 190),
		AccentBlue = Color3.fromRGB(100, 50, 180),
		Blue       = Color3.fromRGB(160, 90, 240),
		Enabled    = Color3.fromRGB(160, 90, 240),
	},
}

function Yerba:SetTheme(preset)
	local overrides = type(preset) == "string" and ThemePresets[preset] or preset
	assert(overrides, "Unknown theme preset: " .. tostring(preset))
	for k, v in pairs(overrides) do
		Theme[k] = v
	end
	-- Live-update all open windows by tweening every descendant that uses theme colors.
	-- The simplest reliable approach: tween Background of all MainWindow frames.
	for _, win in pairs(Yerba.Windows) do
		if win._UIScale and win._UIScale.Parent then
			local mw = win._UIScale.Parent
			if Theme.Background then
				Tween(mw, {BackgroundColor3 = Theme.Background}, 0.3)
			end
		end
	end
end
...
local function IsTouchOrMouse1(input)
...
local function AddStroke(parent, color, thickness)
    ...
    filler.Thickness = (thickness or 0.75) + 1
...
function Yerba:CreateWindow(title, subtitle, options)
	...
	local mobileScale = IsMobile and math.min(1, math.min(screenSize.X, screenSize.Y) / 500) or 1
	local WINDOW_WIDTH = options.Width or math.floor(Yerba.Scale.Window.X.Offset * mobileScale)
	local WINDOW_HEIGHT = options.Height or math.floor(Yerba.Scale.Window.Y.Offset * mobileScale)
	...
	local WindowScale = Create("UIScale", {
		Name = "Scale",
		Scale = Yerba.DPI,  -- inherit global DPI on creation
		Parent = MainWindow
	})

	-- Expose UIScale + per-window DPI so both global and per-window setters work correctly.
	Window._UIScale = WindowScale
	Window._dpi = Yerba.DPI  -- source of truth for this window's scale

	--- Per-window DPI setter.
	function Window:SetDPI(value)
		local scale = NormalizeDPI(value)
		Window._dpi = scale
		ApplyScale(WindowScale, scale)
	end

	--- Per-window DPI getter.
	function Window:GetDPI()
		return Window._dpi
	end
	...
	TopBar.InputBegan:Connect(function(input)
		if IsTouchOrMouse1(input) then
			...
			if not isDragging then
				isDragging = true
				Tween(WindowScale, {Scale = Window._dpi * 1.01}, 0.15, Enum.EasingStyle.Back)
			end
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					...
					if isDragging then
						isDragging = false
						Tween(WindowScale, {Scale = Window._dpi}, 0.2, Enum.EasingStyle.Back)
					end
				end
			end)
		end
	...
	TopBar.InputChanged:Connect(function(input)
		if IsTouchOrMouseMove(input) then
			dragInput = input
		end
	end)
	...
	local function ToggleWindow(show)
		...
		local targetScale = Window._dpi  -- always use the stored DPI, never the live tween value

		if show then
			MainWindow.Visible = true
			WindowScale.Scale = targetScale * 0.9
			MainWindow.BackgroundTransparency = 0.5
			Tween(WindowScale, {Scale = targetScale}, 0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
			Tween(MainWindow, {BackgroundTransparency = 0}, 0.2)
		else
			Tween(WindowScale, {Scale = targetScale * 0.9}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
	...
	function Window:CreateTab(name)
		...
		-- Each column takes exactly half the content area minus half the centre gap.
		-- ScrollingFrame so sections don't clip when there are many of them.
		local COLUMN_GAP = SECTION_GAP * 2
		...
		local LeftColumn = Create("ScrollingFrame", {
			...
			CanvasSize = UDim2.new(0, 0, 0, 0),
			AutomaticCanvasSize = Enum.AutomaticSize.Y,
			Size = UDim2.new(0.5, -math.ceil(COLUMN_GAP / 2), 1, 0),
			Position = UDim2.new(0, 0, 0, 0),
			ZIndex = 1,
			Parent = TabPage
		})
		...
		Create("UIPadding", {
			PaddingTop = UDim.new(0, SECTION_GAP / 2),
			PaddingBottom = UDim.new(0, SECTION_GAP),
			Parent = LeftColumn
		})
		...
		local RightColumn = Create("ScrollingFrame", {
			...
			AutomaticCanvasSize = Enum.AutomaticSize.Y,
			Size = UDim2.new(0.5, -math.ceil(COLUMN_GAP / 2), 1, 0),
			Position = UDim2.new(0.5, math.ceil(COLUMN_GAP / 2), 0, 0),
			ZIndex = 1,
			Parent = TabPage
		})
		Create("UIListLayout", {
		...
		function Tab:CreateSection(sectionName, subtabs, side)
			...
			-- ── SetVisible / SetEnabled helpers injected on every element ──────────
			-- Called internally after each element is created.
			local function InjectCommonAPI(element, frame)
				...
				function element:SetEnabled(enabled)
					...
					for _, desc in pairs(frame:GetDescendants()) do
						if desc:IsA("TextLabel") or desc:IsA("TextButton") or desc:IsA("TextBox") then
							Tween(desc, {TextTransparency = enabled and 0 or 0.55}, 0.15)
						end
						if desc:IsA("ImageLabel") then
							Tween(desc, {ImageTransparency = enabled and 0 or 0.55}, 0.15)
						end
					end
					if not enabled then
						if not frame:FindFirstChild("_DisabledOverlay") then
							Create("TextButton", {
			...
			--[[
				Section:CreateDivider(options)
				  .Label  – optional string shown in the middle (default nil)
				  .Subtab – which subtab to add to
			--]]
			function Section:CreateDivider(options)
				...
				local DividerFrame = Create("Frame", {
					Name = "Divider",
					BackgroundTransparency = 1,
					Size = UDim2.new(1, 0, 0, label and 18 or 8),
					ZIndex = 4,
					Parent = parentContent
				})
				...
				if label then
					...
					Create("Frame", {
						...
						BorderSizePixel = 0,
						Size = UDim2.new(0.35, -4, 0, 1),
						Position = UDim2.new(0.65, 4, 0.5, 0),
						ZIndex = 5,
						Parent = DividerFrame
					})
				else
				...
				local DividerObj = {Frame = DividerFrame}
				InjectCommonAPI(DividerObj, DividerFrame)
				task.defer(UpdateSectionSize)
				return DividerObj
			end
			...
			-- ── ProgressBar ──────────────────────────────────────────────────────
			--[[
				Section:CreateProgressBar(options)
				  .Name    – label
				  .Min     – minimum value (default 0)
				  .Max     – maximum value (default 100)
				  .Default – starting value (default Min)
				  .Suffix  – appended to displayed value (default "")
				  .Color   – bar fill color (default Theme.Blue)
				  .Subtab

				Returns:
				  bar:Set(value)         – update the bar
				  bar:SetMax(max)        – change the max
				  bar:SetLabel(text)     – change name label
				  bar:Get()             – returns current value
			--]]
			function Section:CreateProgressBar(options)
				options = options or {}
				local barName = options.Name or "Progress"
				local min = options.Min or 0
				local max = options.Max or 100
				local default = options.Default or min
				local suffix = options.Suffix or ""
				local barColor = options.Color or Theme.Blue
				local subtab = options.Subtab or Section.CurrentSubtab
				local parentContent = Section.SubtabContents[subtab] and Section.SubtabContents[subtab].Content or GetCurrentContent()
				...
				local Fill = Create("Frame", {
					...
					BorderSizePixel = 0,
					Size = UDim2.new(math.clamp((default - min) / math.max(max - min, 0.001), 0, 1), 0, 1, 0),
					ZIndex = 6,
					Parent = Track
				})
				AddCorner(Fill, 4)
				...
				local function UpdateBar(value)
					value = math.clamp(value, min, max)
					Bar.Value = value
					local pct = math.clamp((value - min) / math.max(max - min, 0.001), 0, 1)
					Tween(Fill, {Size = UDim2.new(pct, 0, 1, 0)}, 0.2)
					ValueLabel.Text = tostring(math.floor(value * 100 + 0.5) / 100) .. suffix
				end

				function Bar:Set(value) UpdateBar(value) end
				function Bar:Get() return Bar.Value end
				function Bar:SetMax(newMax)
					max = newMax
					UpdateBar(Bar.Value)
				end
			...
			-- ── Stepper ───────────────────────────────────────────────────────────
			--[[
				Section:CreateStepper(options)
				  .Name      – label
				  .Options   – array of values to step through (strings or numbers)
				             OR use .Min / .Max / .Increment for numeric range
				  .Default   – starting value or index
				  .Callback  – function(value)
				  .Subtab

				Returns:
				  stepper:Set(value)   – jump to a specific value
				  stepper:Get()        – current value
				  stepper:Next()       – step forward
				  stepper:Prev()       – step back
			--]]
			function Section:CreateStepper(options)
				options = options or {}
				local stepperName = options.Name or "Stepper"
				local callback = options.Callback or function() end
				local subtab = options.Subtab or Section.CurrentSubtab
				local parentContent = Section.SubtabContents[subtab] and Section.SubtabContents[subtab].Content or GetCurrentContent()
				...
				if options.Options then
					...
					local sMax = options.Max or 10
					local sInc = options.Increment or 1
					local v = sMin
					while v <= sMax + 0.0001 do
						table.insert(opts, math.floor(v * 1000 + 0.5) / 1000)
						v = v + sInc
					end
				end

				assert(#opts > 0, "Stepper needs at least one option")

				local Stepper = {}
				local idx = 1
				if options.Default ~= nil then
					for i, v in ipairs(opts) do
						if v == options.Default then idx = i break end
					end
				end
				Stepper.Value = opts[idx]

				local StepFrame = Create("Frame", {
					Name = stepperName,
					BackgroundTransparency = 1,
					Size = UDim2.new(1, 0, 0, 22),
					ZIndex = 4,
				...
				AddCorner(Controls, 5)
				AddStroke(Controls, Theme.Border)
				...
				local function GoTo(i)
					...
					idx = i
					Stepper.Value = opts[idx]
					ValLabel.Text = tostring(opts[idx])
				...
				for _, btn in pairs({PrevBtn, NextBtn}) do
				...
				function Stepper:Set(value)
					for i, v in ipairs(opts) do
						if v == value then GoTo(i) return end
					end
				end
				function Stepper:Get() return Stepper.Value end
				function Stepper:Next() GoTo(idx + 1) end
				function Stepper:Prev() GoTo(idx - 1) end

				Stepper.Frame = StepFrame
				InjectCommonAPI(Stepper, StepFrame)
	...
	-- Mobile toggle button (visible only on touch devices)
	if IsMobile then
		...
		AddCorner(MobileToggleBtn, 8)
		AddStroke(MobileToggleBtn, Theme.BorderAccent)
	...
	--[[
		Window:Notify(options)
		  .Title    – bold heading (default "Notice")
		  .Content  – body text   (default "")
		  .Duration – seconds before auto-dismiss (default 4, pass 0 = forever)
		  .Type     – "info" | "success" | "warning" | "error"  (affects accent color)

		Returns:
		  notif:Dismiss()  – manually close it early
	--]]
	local notifSlots = {}   -- stack of active notification frames (bottom-anchored)
	...
	local NotifColors = {
		info    = Theme.Accent,
		success = Color3.fromRGB(67, 160, 91),
		warning = Color3.fromRGB(200, 155, 50),
		error   = Color3.fromRGB(190, 60, 60),
	}

	local function RebuildNotifPositions()
		local screenH = workspace.CurrentCamera.ViewportSize.Y
		local y = screenH - NOTIF_MARGIN
		for i = #notifSlots, 1, -1 do
			local f = notifSlots[i]
			y = y - NOTIF_HEIGHT
			Tween(f, {Position = UDim2.new(1, -(NOTIF_WIDTH + NOTIF_MARGIN), 0, y)}, 0.2)
			y = y - NOTIF_GAP
		end
	end
	...
	function Window:Notify(options)
		...
		local accentColor = NotifColors[nType] or Theme.Accent

		local screenH = workspace.CurrentCamera.ViewportSize.Y
		local startY  = screenH - NOTIF_MARGIN - NOTIF_HEIGHT
		...
		local NotifFrame = Create("Frame", {
			...
			Position = UDim2.new(1, NOTIF_WIDTH + NOTIF_MARGIN, 0, startY),
			ZIndex = 300,
			Parent = ScreenGui
		})
		AddCorner(NotifFrame, 6)
		AddStroke(NotifFrame, accentColor)
		...
		table.insert(notifSlots, NotifFrame)
		...
		local function Dismiss()
			...
			for i, f in ipairs(notifSlots) do
				if f == NotifFrame then table.remove(notifSlots, i) break end
			end
			RebuildNotifPositions()
			Tween(NotifFrame, {Position = UDim2.new(1, NOTIF_WIDTH + NOTIF_MARGIN, 0, NotifFrame.Position.Y.Offset)}, 0.25)
		...
		NotifFrame.InputBegan:Connect(function(input)
			if IsTouchOrMouse1(input) then Dismiss() end
		end)
	...
	-- ── Minimize ──────────────────────────────────────────────────────────────
	--[[
		Window:Minimize()   – collapse to just the topbar
		Window:Maximize()   – restore to full height
		Window:IsMinimized() – boolean
	--]]
	local isMinimized = false
	local fullHeight = WINDOW_HEIGHT
	...
	AddContextualStroke(MinimizeBtn)

	MinimizeBtn.MouseEnter:Connect(function()
		Tween(MinimizeBtn, {TextColor3 = Theme.Text}, 0.1)
	end)
	MinimizeBtn.MouseLeave:Connect(function()
		Tween(MinimizeBtn, {TextColor3 = Theme.TextDark}, 0.1)
	end)

	local function DoMinimize()
		isMinimized = true
		MinimizeBtn.Text = "□"
		Tween(MainWindow, {Size = UDim2.new(0, WINDOW_WIDTH, 0, TOPBAR_HEIGHT)}, 0.2, Enum.EasingStyle.Quad)
	end

	local function DoMaximize()
		isMinimized = false
		MinimizeBtn.Text = "─"
		Tween(MainWindow, {Size = UDim2.new(0, WINDOW_WIDTH, 0, fullHeight)}, 0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
	end

	MinimizeBtn.MouseButton1Click:Connect(function()
		if isMinimized then DoMaximize() else DoMinimize() end
	end)

	function Window:Minimize() DoMinimize() end
	function Window:Maximize() DoMaximize() end
	function Window:IsMinimized() return isMinimized end
