...
Compkiller._DPI = 1.0;
Compkiller._UIScaleRef = nil;

function Compkiller:SetDPI(scale)
	assert(type(scale)=="number" and scale>0,"SetDPI: scale must be positive");
	self._DPI = scale;
	if self._UIScaleRef then
		self:_Animation(self._UIScaleRef, TweenInfo.new(0.3,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Scale=scale});
	end;
end;

function Compkiller:SetAccentColor(color)
	Compkiller.Colors.Highlight = color;
	Compkiller.Colors.Toggle    = color;
	for _,v in next, Compkiller.Elements.Highlight do
		pcall(function() v.Element[v.Property]=color end);
	end;
end;

function Compkiller:_InitDPI(screenGui)
	local s = Instance.new("UIScale");
	s.Scale = self._DPI; s.Parent = screenGui;
	self._UIScaleRef = s; return s;
end;
...
function Compkiller:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	function Args:AddProgressBar(Config)
		Config=Compkiller.__CONFIG(Config,{Name="Progress",Value=0,Max=100,Color=Compkiller.Colors.Highlight,ShowText=true,Type="%"})
		local PB=Instance.new("Frame"); local PBLbl=Instance.new("TextLabel")
		local PBLine=Instance.new("Frame"); local PBTrack=Instance.new("Frame")
		local PBFill=Instance.new("Frame"); local PBVal=Instance.new("TextLabel")
		local C1=Instance.new("UICorner"); local C2=Instance.new("UICorner")
		PB.Name=Compkiller:_RandomString(); PB.Parent=Parent; PB.BackgroundTransparency=1
		PB.BorderSizePixel=0; PB.Size=UDim2.new(1,-1,0,45); PB.ZIndex=Zindex+1
		PBLbl.Name=Compkiller:_RandomString(); PBLbl.Parent=PB; PBLbl.BackgroundTransparency=1
		PBLbl.BorderSizePixel=0; PBLbl.Position=UDim2.new(0,12,0,1); PBLbl.Size=UDim2.new(1,-20,0,25)
		PBLbl.ZIndex=Zindex+2; PBLbl.Font=Enum.Font.GothamMedium; PBLbl.Text=Config.Name
		PBLbl.TextColor3=Compkiller.Colors.SwitchColor; PBLbl.TextSize=14; PBLbl.TextTransparency=0.1
		PBLbl.TextXAlignment=Enum.TextXAlignment.Left
		table.insert(Compkiller.Elements.SwitchColor,{Element=PBLbl,Property="TextColor3"})
		PBLine.Name=Compkiller:_RandomString(); PBLine.Parent=PB; PBLine.AnchorPoint=Vector2.new(0.5,1)
		PBLine.BackgroundColor3=Compkiller.Colors.LineColor; PBLine.BackgroundTransparency=0.5
		PBLine.BorderSizePixel=0; PBLine.Position=UDim2.new(0.5,0,1,0); PBLine.Size=UDim2.new(1,-26,0,1)
		PBLine.ZIndex=Zindex+2
		table.insert(Compkiller.Elements.LineColor,{Element=PBLine,Property="BackgroundColor3"})
		PBTrack.Name=Compkiller:_RandomString(); PBTrack.Parent=PB; PBTrack.AnchorPoint=Vector2.new(0.5,1)
		PBTrack.BackgroundColor3=Compkiller.Colors.DropColor; PBTrack.BorderSizePixel=0
		PBTrack.ClipsDescendants=true; PBTrack.Position=UDim2.new(0.5,0,1,-9)
		PBTrack.Size=UDim2.new(1,-25,0,10); PBTrack.ZIndex=Zindex+3
		table.insert(Compkiller.Elements.DropColor,{Element=PBTrack,Property="BackgroundColor3"})
		C1.CornerRadius=UDim.new(0,6); C1.Parent=PBTrack
		PBFill.Name=Compkiller:_RandomString(); PBFill.Parent=PBTrack; PBFill.AnchorPoint=Vector2.new(0,0.5)
		PBFill.BackgroundColor3=Config.Color; PBFill.BorderSizePixel=0
		PBFill.Position=UDim2.new(0,0,0.5,0); PBFill.ZIndex=Zindex+4
		PBFill.Size=UDim2.new(math.clamp(Config.Value/Config.Max,0,1),0,1,0)
		C2.CornerRadius=UDim.new(0,6); C2.Parent=PBFill
		PBVal.Name=Compkiller:_RandomString(); PBVal.Parent=PB; PBVal.BackgroundTransparency=1
		PBVal.BorderSizePixel=0; PBVal.Position=UDim2.new(0,12,0,1); PBVal.Size=UDim2.new(1,-20,0,25)
		PBVal.ZIndex=Zindex+4; PBVal.Font=Enum.Font.GothamMedium
		PBVal.Text=tostring(Config.Value)..Config.Type
		PBVal.TextColor3=Compkiller.Colors.SwitchColor; PBVal.TextSize=12
		PBVal.TextTransparency=0.65; PBVal.TextXAlignment=Enum.TextXAlignment.Right
		table.insert(Compkiller.Elements.SwitchColor,{Element=PBVal,Property="TextColor3"})
		Signal:Connect(function(b)
			local t=b and 0 or 1
			Compkiller:_Animation(PBFill,Tween,{BackgroundTransparency=t})
			Compkiller:_Animation(PBTrack,Tween,{BackgroundTransparency=t})
			Compkiller:_Animation(PBLbl,Tween,{TextTransparency=b and 0.1 or 1})
			Compkiller:_Animation(PBLine,Tween,{BackgroundTransparency=b and 0.5 or 1})
			Compkiller:_Animation(PBVal,Tween,{TextTransparency=b and 0.65 or 1})
		end)
		local API={}
		function API:SetValue(val)
			Config.Value=math.clamp(val,0,Config.Max)
			Compkiller:_Animation(PBFill,TweenInfo.new(0.35,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{
				Size=UDim2.new(Config.Value/Config.Max,0,1,0)
			})
			if Config.ShowText then PBVal.Text=tostring(math.round(Config.Value))..Config.Type end
		end
		function API:GetValue() return Config.Value end
		function API:SetMax(m) Config.Max=m; API:SetValue(Config.Value) end
		function API:SetColor(c) Config.Color=c; PBFill.BackgroundColor3=c end
		function API:SetText(t) PBLbl.Text=t end
		function API:GetText() return PBLbl.Text end
		function API:AnimateTo(target,duration)
			local s=Config.Value; target=math.clamp(target,0,Config.Max)
			duration=duration or 1; local t0=tick()
			task.spawn(function()
				repeat task.wait()
					local a=math.clamp((tick()-t0)/duration,0,1)
					API:SetValue(s+(target-s)*a)
				until a>=1
			end)
		end
		return API
...
function Compkiller.new(Config : Window)
	...
	Compkiller:_InitDPI(CompKiller);
	...
	-- === Bottom Tag Bar ===
	local CK_BB = Instance.new("Frame")
	local CK_BL = Instance.new("Frame")
	local CK_TS = Instance.new("ScrollingFrame")
	local CK_TL = Instance.new("UIListLayout")
	local CK_BC = Instance.new("UICorner")
	CK_BB.Name="CK_BottomBar"; CK_BB.Parent=MainFrame
	CK_BB.AnchorPoint=Vector2.new(0.5,1); CK_BB.BackgroundColor3=Compkiller.Colors.BGDBColor
	CK_BB.BackgroundTransparency=0.35; CK_BB.BorderSizePixel=0
	CK_BB.Position=UDim2.new(0.5,0,1,0); CK_BB.Size=UDim2.new(1,0,0,22)
	CK_BB.ZIndex=10; CK_BB.ClipsDescendants=true
	CK_BC.CornerRadius=UDim.new(0,6); CK_BC.Parent=CK_BB
	table.insert(Compkiller.Elements.BGDBColor,{Element=CK_BB,Property="BackgroundColor3"})
	CK_BL.Parent=CK_BB; CK_BL.AnchorPoint=Vector2.new(0.5,0); CK_BL.BackgroundColor3=Compkiller.Colors.StrokeColor
	CK_BL.BorderSizePixel=0; CK_BL.Position=UDim2.new(0.5,0,0,0); CK_BL.Size=UDim2.new(1,-20,0,1); CK_BL.ZIndex=11
	table.insert(Compkiller.Elements.StrokeColor,{Element=CK_BL,Property="BackgroundColor3"})
	CK_TS.Parent=CK_BB; CK_TS.AnchorPoint=Vector2.new(0,0.5); CK_TS.BackgroundTransparency=1
	CK_TS.BorderSizePixel=0; CK_TS.Position=UDim2.new(0,8,0.5,1); CK_TS.Size=UDim2.new(1,-16,1,-4)
	CK_TS.ZIndex=11; CK_TS.ScrollBarThickness=0; CK_TS.BottomImage=""; CK_TS.TopImage=""
	CK_TS.ScrollingDirection=Enum.ScrollingDirection.X
	CK_TL.Parent=CK_TS; CK_TL.FillDirection=Enum.FillDirection.Horizontal
	CK_TL.VerticalAlignment=Enum.VerticalAlignment.Center; CK_TL.SortOrder=Enum.SortOrder.LayoutOrder
	CK_TL.Padding=UDim.new(0,5)
	CK_TL:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
		CK_TS.CanvasSize=UDim2.fromOffset(CK_TL.AbsoluteContentSize.X+10,0)
	end)
	WindowArgs.BottomBar=CK_BB; WindowArgs.TagScroll=CK_TS; WindowArgs._TagLayout=CK_TL; WindowArgs._Tags={}
	function WindowArgs:AddTag(C)
		C=Compkiller.__CONFIG(C or {},{Text="Tag",Color=Compkiller.Colors.Highlight})
		local tf=Instance.new("Frame"); local tc=Instance.new("UICorner")
		local tp=Instance.new("UIPadding"); local tl=Instance.new("TextLabel")
		tf.Parent=CK_TS; tf.BackgroundColor3=C.Color; tf.BackgroundTransparency=0.82
		tf.BorderSizePixel=0; tf.AutomaticSize=Enum.AutomaticSize.X
		tf.Size=UDim2.new(0,0,1,-4); tf.ZIndex=12
		tc.CornerRadius=UDim.new(0,3); tc.Parent=tf
		tp.PaddingLeft=UDim.new(0,6); tp.PaddingRight=UDim.new(0,6); tp.Parent=tf
		tl.Parent=tf; tl.BackgroundTransparency=1; tl.Size=UDim2.new(1,0,1,0)
		tl.ZIndex=13; tl.Font=Enum.Font.GothamMedium; tl.Text=C.Text
		tl.TextColor3=C.Color; tl.TextSize=9; tl.TextTransparency=0.1
		tl.TextXAlignment=Enum.TextXAlignment.Center
		local A={}
		function A:SetText(t) tl.Text=t end
		function A:SetColor(c) tf.BackgroundColor3=c; tl.TextColor3=c end
		function A:SetVisible(v) tf.Visible=v end
		function A:GetText() return tl.Text end
		function A:Remove()
			Compkiller:_Animation(tf,TweenInfo.new(0.2),{BackgroundTransparency=1})
			task.delay(0.25,function() tf:Destroy() end)
		end
		table.insert(WindowArgs._Tags,A); return A
	end
	-- === End Bottom Tag Bar ===
	...
	-- === WindowArgs:SetDPI + Built-in Settings Tab ===
	function WindowArgs:SetDPI(scale)
		Compkiller:SetDPI(scale);
	end;
	if not Config.DisableSettingsTab then
		local _st = WindowArgs:DrawTab({ Name="Settings", Icon="settings-3" });
		if _st then
			pcall(function()
				local s = _st:DrawSection({ Name="Interface" })
				local cat = s:DrawCategory({ Name="Left" })
				cat:AddSlider({ Name="UI Scale (DPI)", Min=0.5, Max=2.0, Default=Compkiller._DPI, Round=2, Type="x", Callback=function(v) Compkiller:SetDPI(v) end })
				cat:AddColorPicker({ Name="Accent Color", Default=Compkiller.Colors.Highlight, Callback=function(c) Compkiller:SetAccentColor(c) end })
				cat:AddToggle({ Name="Performance Mode", Default=false, Callback=function(v) WindowArgs.PerformanceMode=v end })
				cat:AddParagraph({ Title="Compkiller v3.0", Content="SetDPI · Tags · ProgressBar · Settings" })
			end)
		end
	end
	-- === End Settings Tab ===
