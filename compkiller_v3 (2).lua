...
-- ── CK ENHANCED: Connection / cleanup registry ───────────────────────────────
Compkiller._CK_Connections = {};

--- Register an RBXScriptConnection so it can be batch-cleaned with Cleanup().
function Compkiller:_RegisterConnection(conn)
	if conn and typeof(conn) == 'RBXScriptConnection' then
		table.insert(Compkiller._CK_Connections, conn);
	end;
	return conn;
end;

--- Disconnect all registered connections and destroy cached notify/window GUIs.
--- Call this when your script unloads to avoid memory leaks.
function Compkiller:Cleanup()
	for _, conn in next , Compkiller._CK_Connections do
		pcall(function() conn:Disconnect() end);
	end;
	table.clear(Compkiller._CK_Connections);

	for _, win in next , Compkiller.Windows do
		pcall(function() win:Destroy() end);
	end;
	table.clear(Compkiller.Windows);

	-- cancel any running task threads
	if Compkiller._ACTIVE_THREADS then
		for _, t in next , Compkiller._ACTIVE_THREADS do
			pcall(function() task.cancel(t) end);
		end;
		table.clear(Compkiller._ACTIVE_THREADS);
	end;

	if Compkiller.NOTIFY_CACHE then
		Compkiller.NOTIFY_CACHE = nil;
	end;
...
--- Returns a shallow copy of all registered flags and their current values.
function Compkiller:GetAllFlags() : {[string] : any}
	local out = {};
	for k,v in next , Compkiller.Flags do
		if v and v.GetValue then
			local ok, val = pcall(function() return v:GetValue() end);
			out[k] = ok and val or nil;
		end;
	end;
	return out;
end;

--- Resets a single flag to its provided value (or calls SetValue(nil) to clear).
--- @param flag string
--- @param value any?
function Compkiller:ResetFlag(flag : string , value : any?)
	local f = Compkiller.Flags[flag];
	if f and f.SetValue then
		pcall(function() f:SetValue(value) end);
	end;
end;

--- Calls SetValue on every registered flag. Pass a table to reset to specific
--- values, or nothing to skip (use alongside ResetFlag for bulk resets).
--- @param values {[string]:any}?
function Compkiller:ResetAll(values : {[string]:any}?)
	values = values or {};
	for k,v in next , Compkiller.Flags do
		local target = values[k];
		if v and v.SetValue and target ~= nil then
			pcall(function() v:SetValue(target) end);
		end;
	end;
end;
...
function Compkiller:_AddDragBlacklist(Frame: Frame)
	...
	local _bl_conn = UserInputService.InputChanged:Connect(function()
		if not Compkiller:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(false);
		end;
	end);

	-- CK v3: auto-disconnect on destroy to prevent memory leaks
	table.insert(Compkiller._CK_Connections, _bl_conn);
	Frame.Destroying:Connect(function()
		pcall(function() _bl_conn:Disconnect() end);
		SET_BLACKLIST(false);
	end);
end;
...
function Compkiller:_GetWindowFromElement(Element)
	...
	for i,v : ScreenGui in next , Compkiller.Windows do
		if v and pcall(function() return Element:IsDescendantOf(v) end) then
			local ok, res = pcall(function() return Element:IsDescendantOf(v) end);
			if ok and res then return v; end;
		end;
	end;
end;
...
function Compkiller:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	-- ── CK v3: improved drag – bounds clamping + mobile velocity throw ───────
	local dragToggle  : boolean = false;
	local dragStart   : Vector3 = nil;
	local startPos    : UDim2   = nil;
	local Tween       = TweenInfo.new(Speed or 0.1);

	-- velocity tracking for throw on mobile
	local lastPos     : Vector3 = Vector3.zero;
	local lastTick    : number  = 0;
	local velocity    : Vector2 = Vector2.zero;

	local function clampToScreen(pos : UDim2) : UDim2
		local vp   = workspace.CurrentCamera.ViewportSize;
		local size = MoveFrame.AbsoluteSize;
		local ox   = math.clamp(pos.X.Offset, 0, math.max(0, vp.X - size.X));
		local oy   = math.clamp(pos.Y.Offset, 0, math.max(0, vp.Y - size.Y));
		return UDim2.new(pos.X.Scale, ox, pos.Y.Scale, oy);
	end;

	local function updateInput(input)
		if not dragStart then return end;
		local delta   = input.Position - dragStart;
		local rawPos  = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X,
		                          startPos.Y.Scale,  startPos.Y.Offset + delta.Y);
		local clamped = clampToScreen(rawPos);

		-- track velocity for mobile throw
		local now = tick();
		if now - lastTick > 0 then
			velocity = Vector2.new(
				(input.Position.X - lastPos.X) / (now - lastTick),
				(input.Position.Y - lastPos.Y) / (now - lastTick)
			);
		end;
		lastPos  = input.Position;
		lastTick = now;

		Compkiller:_Animation(MoveFrame, Tween, { Position = clamped });
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1
		 or input.UserInputType == Enum.UserInputType.Touch)
		 and #Compkiller.DragBlacklist <= 0 then
			dragToggle = true;
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					...
					-- mobile: apply a brief velocity-based throw, then clamp
					if UserInputService.TouchEnabled and velocity.Magnitude > 50 then
						local cur = MoveFrame.Position;
						local throwPos = clampToScreen(UDim2.new(
							cur.X.Scale, cur.X.Offset + velocity.X * 0.15,
							cur.Y.Scale, cur.Y.Offset + velocity.Y * 0.15
						));
						Compkiller:_Animation(MoveFrame, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
							Position = throwPos
						});
					end;
				end;
		...
		if not Compkiller.IaDrag and dragToggle then
			Compkiller.LastDrag = tick();
		end;
	...
	UserInputService.InputChanged:Connect(function(input)
		...
		if (isMouse or isTouch) and #Compkiller.DragBlacklist <= 0 then
			if dragToggle then
...
function Compkiller:CacheImage(id: string) : string
	...
	if isfile(cache_path) then
		return (getcustomasset or getsynasset)(cache_path);
	end;
...
function Compkiller:_DrawKeybinds(Window: ScreenGui)
	...
	function Ref:AddFrame()
		...
		Keyholder.BackgroundTransparency = 0.600
...
function Compkiller:_KeybindHandler(Parent: Frame , ObjectType: string , ElementAPI: Toggle & Slider , Signal , Zindex: number , ElementCFG: Slider)
	...
	KeybindHandler.Name = "KeybindHandler"
...
function Compkiller:_LoadOption(Value , TabSignal)
	...
	function Args:AddKeybind(Config: MiniKeybind)
		...
		Compkiller:_Input(Keybind.Root,function()
			...
			pcall(function() Config.Callback(KeyName) end);
		end);
		...
		function Args:SetValue(value)
			...
			pcall(function() Config.Callback(Config.Default) end);
		end;
	...
	function Args:AddColorPicker(Config: MiniColorPicker)
		...
		local ColorPicker = Compkiller:_AddColorPickerPanel(Button,function(color,opc)
			...
			pcall(function() Config.Callback(Config.Default , Config.Transparency) end);
		end);
		...
		function Args:SetValue(value,opc)
			...
			pcall(function() Config.Callback(value,opc) end);
		end;
...
function Compkiller:_LoadDropdown(BaseParent: TextButton , Callback: () -> any)
	...
	-- ── CK ENHANCED: Dropdown Search Box ──────────────────────────────────────
	local SearchQuery = "";
	...
	SearchHolder.Name               = Compkiller:_RandomString()
	SearchHolder.Parent             = DropdownWindow
	SearchHolder.BackgroundColor3   = Compkiller.Colors.DropColor
	SearchHolder.BorderSizePixel    = 0
	SearchHolder.ClipsDescendants   = true
	SearchHolder.Size               = UDim2.new(1, 0, 0, 0)
	SearchHolder.ZIndex             = BaseZ_Index + 8
	...
	SearchBox.Name                  = Compkiller:_RandomString()
	...
	SearchBox.PlaceholderColor3     = Compkiller.Colors.SwitchColor
	SearchBox.Text                  = ""
	SearchBox.TextColor3            = Compkiller.Colors.SwitchColor
	SearchBox.TextSize              = 12
	SearchBox.TextXAlignment        = Enum.TextXAlignment.Left
	SearchBox.Position              = UDim2.new(0, 22, 0, 0)
	SearchBox.Size                  = UDim2.new(1, -27, 1, 0)
	SearchBox.ZIndex                = BaseZ_Index + 10

	table.insert(Compkiller.Elements.SwitchColor, {Element = SearchBox, Property = "TextColor3"});
	table.insert(Compkiller.Elements.SwitchColor, {Element = SearchBox, Property = "PlaceholderColor3"});

	SearchIcon.Name                  = Compkiller:_RandomString()
	SearchIcon.Parent                = SearchHolder
	SearchIcon.BackgroundTransparency = 1
	SearchIcon.BorderSizePixel       = 0
	SearchIcon.Image                 = Compkiller:_GetIcon("lucide-search") or "rbxassetid://10723420826"
	SearchIcon.ImageTransparency     = 0.4
	SearchIcon.Position              = UDim2.new(0, 5, 0.5, -7)
	SearchIcon.Size                  = UDim2.new(0, 14, 0, 14)
	SearchIcon.ZIndex                = BaseZ_Index + 10

	SearchLine.Name                  = Compkiller:_RandomString()
	SearchLine.Parent                = SearchHolder
	SearchLine.AnchorPoint           = Vector2.new(0.5, 1)
	SearchLine.BackgroundColor3      = Compkiller.Colors.LineColor
	SearchLine.BackgroundTransparency = 0.4
	SearchLine.BorderSizePixel       = 0
	SearchLine.Position              = UDim2.new(0.5, 0, 1, 0)
	SearchLine.Size                  = UDim2.new(0.9, 0, 0, 1)
	SearchLine.ZIndex                = BaseZ_Index + 9

	table.insert(Compkiller.Elements.LineColor, {Element = SearchLine, Property = "BackgroundColor3"});
	...
	SearchBox:GetPropertyChangedSignal('Text'):Connect(function()
		...
		for _, child in next , ScrollingFrame:GetChildren() do
			if child:IsA('Frame') then
				...
				if lbl then
					local matches = SearchQuery == "" or lbl.Text:lower():find(SearchQuery, 1, true);
					child.Visible = matches and true or false;
				end;
			end;
		end;
	end);

	-- show / hide search when dropdown opens / closes
	local _origToggleUI_ref; -- patched below via wrapper
	-- ── END CK Search Box setup ──────────────────────────────────────────────
	...
	-- ── CK ENHANCED: wire search holder visibility into ToggleUI ────────────
	local _ck_origToggleUI = ToggleUI;
	ToggleUI = function(bool)
		_ck_origToggleUI(bool);
		if bool and Values and #Values > 5 then
			_SearchEnabled = true;
			SearchHolder.BackgroundTransparency = 0;
			SearchHolder.Size = UDim2.new(1, 0, 0, 24);
			ScrollingFrame.Size = UDim2.new(1, -5, 1, -29);
			ScrollingFrame.Position = UDim2.new(0.5, 0, 0, 27);
		else
...
function Compkiller:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
	...
	-- ── CK v3: Element visibility helpers ─────────────────────────────────────
	--- Hide an element's root frame without destroying it.
	function Args:HideElement(Root : Frame)
		if Root then
			pcall(function()
				Root.Visible = false;
			end);
	...
	function Args:ShowElement(Root : Frame)
		if Root then
			pcall(function()
				Root.Visible = true;
			end);
	...
	--- Completely destroy an element's root and unregister its flag.
	function Args:DestroyElement(elem)
		if not elem then return; end;
		-- unregister flag
		if elem.Flag then
			Compkiller.Flags[elem.Flag] = nil;
		end;
		-- disconnect signal if present
		if elem.Signal and typeof(elem.Signal) == 'RBXScriptConnection' then
			pcall(function() elem.Signal:Disconnect(); end);
		end;
	...
	function Args:AddProgressBar(Config)
		Config = Compkiller.__CONFIG(Config, {
			Name     = "Progress",
			Value    = 0,    -- 0-100
			Max      = 100,
			Color    = nil,  -- defaults to Highlight
			Flag     = nil,
			Callback = function() end,
		});

		Config.Value = math.clamp(Config.Value or 0, 0, Config.Max);
		...
		local UIStroke_BG    = Instance.new("UIStroke")
		local BarFill        = Instance.new("Frame")
		local UICorner_Fill  = Instance.new("UICorner")
		local ValueLabel     = Instance.new("TextLabel")

		if Compkiller:_IsMobile() then
			Compkiller:_AddDragBlacklist(ProgressFrame);
		end;

		ProgressFrame.Name                = Compkiller:_RandomString()
		ProgressFrame.Parent              = Parent
		ProgressFrame.BackgroundTransparency = 1
		ProgressFrame.BorderSizePixel     = 0
		ProgressFrame.Size                = UDim2.new(1, -1, 0, 45)
		ProgressFrame.ZIndex              = Zindex + 1

		BlockText.Name                    = Compkiller:_RandomString()
		BlockText.Parent                  = ProgressFrame
		...
		BlockLine.Name                    = Compkiller:_RandomString()
		BlockLine.Parent                  = ProgressFrame
		BlockLine.AnchorPoint             = Vector2.new(0.5, 1)
		BlockLine.BackgroundColor3        = Compkiller.Colors.LineColor
		...
		BarBG.Name                        = Compkiller:_RandomString()
		BarBG.Parent                      = ProgressFrame
		BarBG.AnchorPoint                 = Vector2.new(0.5, 1)
		BarBG.BackgroundColor3            = Compkiller.Colors.DropColor
		BarBG.BorderSizePixel             = 0
		BarBG.ClipsDescendants            = true
		BarBG.Position                    = UDim2.new(0.5, 0, 1, -9)
		BarBG.Size                        = UDim2.new(1, -25, 0, 10)
		BarBG.ZIndex                      = Zindex + 3

		table.insert(Compkiller.Elements.DropColor, {Element = BarBG, Property = "BackgroundColor3"});

		UICorner_BG.CornerRadius          = UDim.new(0, 6)
		UICorner_BG.Parent                = BarBG

		UIStroke_BG.Color                 = Compkiller.Colors.StrokeColor
		UIStroke_BG.Parent                = BarBG

		table.insert(Compkiller.Elements.StrokeColor, {Element = UIStroke_BG, Property = "Color"});

		BarFill.Name                      = Compkiller:_RandomString()
		BarFill.Parent                    = BarBG
		BarFill.AnchorPoint               = Vector2.new(0, 0.5)
		BarFill.BackgroundColor3          = Config.Color or Compkiller.Colors.Highlight
		BarFill.BorderSizePixel           = 0
		BarFill.Position                  = UDim2.new(0, 0, 0.5, 0)
		BarFill.Size                      = UDim2.new(math.max(Config.Value / Config.Max, 0.02), 0, 1, 0)
		BarFill.ZIndex                    = Zindex + 4

		if not Config.Color then
			table.insert(Compkiller.Elements.Highlight, {Element = BarFill, Property = "BackgroundColor3"});
		end;

		UICorner_Fill.CornerRadius        = UDim.new(0, 6)
		UICorner_Fill.Parent              = BarFill

		ValueLabel.Name                   = Compkiller:_RandomString()
		ValueLabel.Parent                 = ProgressFrame
		ValueLabel.BackgroundTransparency = 1
		ValueLabel.BorderSizePixel        = 0
		ValueLabel.Position               = UDim2.new(0, 12, 0, 1)
		ValueLabel.Size                   = UDim2.new(1, -20, 0, 25)
		ValueLabel.ZIndex                 = Zindex + 3
		ValueLabel.Font                   = Enum.Font.GothamMedium
		ValueLabel.Text                   = tostring(math.floor(Config.Value)).." / "..tostring(Config.Max)
		ValueLabel.TextColor3             = Compkiller.Colors.SwitchColor
		ValueLabel.TextSize               = 12
		ValueLabel.TextTransparency       = 0.5
		ValueLabel.TextXAlignment         = Enum.TextXAlignment.Right

		table.insert(Compkiller.Elements.SwitchColor, {Element = ValueLabel, Property = "TextColor3"});

		Signal:Connect(function(bool)
			if bool then
				Compkiller:_Animation(BlockText,  Tween, {TextTransparency = 0.1});
				Compkiller:_Animation(BlockLine,  Tween, {BackgroundTransparency = 0.5});
				Compkiller:_Animation(BarBG,      Tween, {BackgroundTransparency = 0});
				Compkiller:_Animation(UIStroke_BG,Tween, {Transparency = 0});
				Compkiller:_Animation(BarFill,    Tween, {BackgroundTransparency = 0});
				Compkiller:_Animation(ValueLabel, Tween, {TextTransparency = 0.5});
			else
				Compkiller:_Animation(BlockText,  Tween, {TextTransparency = 1});
				Compkiller:_Animation(BlockLine,  Tween, {BackgroundTransparency = 1});
				Compkiller:_Animation(BarBG,      Tween, {BackgroundTransparency = 1});
				Compkiller:_Animation(UIStroke_BG,Tween, {Transparency = 1});
				Compkiller:_Animation(BarFill,    Tween, {BackgroundTransparency = 1});
				Compkiller:_Animation(ValueLabel, Tween, {TextTransparency = 1});
			end;
		end);

		local ProgressArgs = {};
		ProgressArgs.Flag = Config.Flag;

		--- Set the current progress value (0 .. Max). Animates the fill bar.
		function ProgressArgs:SetValue(val : number)
			Config.Value = math.clamp(val or 0, 0, Config.Max);

			local scale = math.max(Config.Value / Config.Max, 0.02);

			Compkiller:_Animation(BarFill, TweenInfo.new(0.4, Enum.EasingStyle.Quint), {
				Size = UDim2.new(scale, 0, 1, 0)
			});

			ValueLabel.Text = tostring(math.floor(Config.Value)).." / "..tostring(Config.Max);

			pcall(function() Config.Callback(Config.Value) end);
		end;

		--- Get the current value.
		function ProgressArgs:GetValue()
			return Config.Value;
		end;

		--- Update the label text.
		function ProgressArgs:SetText(str : string)
			BlockText.Text = str or Config.Name;
		end;

		function ProgressArgs:GetText()
			return BlockText.Text;
		end;

		if Config.Flag then
			Compkiller.Flags[Config.Flag] = ProgressArgs;
		end;
	...
	-- ── CK v3: AddDivider ────────────────────────────────────────────────────
	--- A simple horizontal rule with optional label text.
	--- @param Config { Name:string?, Color:Color3? }
	function Args:AddDivider(Config)
		Config = Compkiller.__CONFIG(Config or {}, {
			Name  = "",
			Color = nil,
		});
		...
		Divider.Size                = UDim2.new(1, -1, 0, 18)
		Divider.ZIndex              = Zindex + 1

		DivLine.Name                = Compkiller:_RandomString()
		DivLine.Parent              = Divider
		DivLine.AnchorPoint         = Vector2.new(0.5, 0.5)
		DivLine.BackgroundColor3    = Config.Color or Compkiller.Colors.LineColor
		DivLine.BackgroundTransparency = 0.3
		DivLine.BorderSizePixel     = 0
		DivLine.Position            = UDim2.new(0.5, 0, 0.5, 0)
		DivLine.Size                = UDim2.new(1, -26, 0, 1)
		DivLine.ZIndex              = Zindex + 2

		if not Config.Color then
			table.insert(Compkiller.Elements.LineColor, {Element=DivLine, Property="BackgroundColor3"});
		end;

		DivLabel.Name               = Compkiller:_RandomString()
		DivLabel.Parent             = Divider
		DivLabel.AnchorPoint        = Vector2.new(0.5, 0.5)
		DivLabel.BackgroundColor3   = Compkiller.Colors.BlockColor
		DivLabel.BackgroundTransparency = (Config.Name == "") and 1 or 0
		DivLabel.BorderSizePixel    = 0
		DivLabel.Position           = UDim2.new(0.5, 0, 0.5, 0)
		DivLabel.Size               = UDim2.new(0, 0, 1, 0)
		DivLabel.ZIndex             = Zindex + 3
		DivLabel.Font               = Enum.Font.Gotham
		DivLabel.Text               = Config.Name
		DivLabel.TextColor3         = Compkiller.Colors.SwitchColor
		DivLabel.TextSize           = 11
		DivLabel.TextTransparency   = 0.45

		table.insert(Compkiller.Elements.BlockColor,  {Element=DivLabel, Property="BackgroundColor3"});
		table.insert(Compkiller.Elements.SwitchColor, {Element=DivLabel, Property="TextColor3"});

		if Config.Name ~= "" then
			local ts = TextService:GetTextSize(Config.Name, 11, Enum.Font.Gotham, Vector2.new(math.huge,math.huge));
			DivLabel.Size = UDim2.new(0, ts.X + 12, 1, 0);
		end;

		Signal:Connect(function(bool)
			Compkiller:_Animation(DivLine,  Tween, {BackgroundTransparency = bool and 0.3 or 1});
			Compkiller:_Animation(DivLabel, Tween, {TextTransparency       = bool and 0.45 or 1});
		end);

		local DivArgs = {};
		function DivArgs:SetText(str)
			Config.Name   = str or "";
			DivLabel.Text = Config.Name;
			if Config.Name ~= "" then
				local ts = TextService:GetTextSize(Config.Name,11,Enum.Font.Gotham,Vector2.new(math.huge,math.huge));
				DivLabel.Size = UDim2.new(0, ts.X+12, 1, 0);
				DivLabel.BackgroundTransparency = 0;
			else
				DivLabel.Size = UDim2.new(0,0,1,0);
	...
	-- ── CK v3: AddLabel (styled text badge) ──────────────────────────────────
	--- @param Config { Name:string, Content:string?, Icon:string?, Color:Color3?, Flag:string? }
	function Args:AddLabel(Config)
		Config = Compkiller.__CONFIG(Config or {}, {
			Name    = "Label",
			Content = "",
			Icon    = "",
		...
		if Compkiller:_IsMobile() then Compkiller:_AddDragBlacklist(LabelFrame); end;

		local accentColor = Config.Color or Compkiller.Colors.Highlight;
		local hasIcon     = Config.Icon ~= "";
		local hasContent  = Config.Content ~= "";
		local frameH      = hasContent and 48 or 30;

		LabelFrame.Name                = Compkiller:_RandomString()
		LabelFrame.Parent              = Parent
		LabelFrame.BackgroundTransparency = 1
		LabelFrame.BorderSizePixel     = 0
		LabelFrame.Size                = UDim2.new(1,-1, 0, frameH)
		LabelFrame.ZIndex              = Zindex + 1

		BG.Name                        = Compkiller:_RandomString()
		BG.Parent                      = LabelFrame
		BG.BackgroundColor3            = accentColor
		BG.BackgroundTransparency      = 0.88
		BG.BorderSizePixel             = 0
		BG.Position                    = UDim2.new(0, 12, 0.5, 0)
		BG.AnchorPoint                 = Vector2.new(0, 0.5)
		BG.Size                        = UDim2.new(1, -24, 1, -8)
		BG.ZIndex                      = Zindex + 2
		UICornerBG.CornerRadius        = UDim.new(0, 4)
		UICornerBG.Parent              = BG
		UIStrokeBG.Color               = accentColor
		UIStrokeBG.Transparency        = 0.6
		UIStrokeBG.Parent              = BG

		local leftOff = hasIcon and 28 or 10;

		if hasIcon then
			IconImg.Name               = Compkiller:_RandomString()
			IconImg.Parent             = BG
			IconImg.BackgroundTransparency = 1
			IconImg.AnchorPoint        = Vector2.new(0, 0.5)
			IconImg.Position           = UDim2.new(0, 7, 0.5, 0)
			IconImg.Size               = UDim2.new(0, 14, 0, 14)
			IconImg.ZIndex             = Zindex + 3
			IconImg.Image              = Compkiller:_GetIcon(Config.Icon) or ""
			IconImg.ImageColor3        = accentColor
		end;

		TitleLbl.Name                  = Compkiller:_RandomString()
		TitleLbl.Parent                = BG
		...
		TitleLbl.Position              = UDim2.new(0, leftOff, 0, hasContent and 4 or 0)
		TitleLbl.Size                  = UDim2.new(1, -(leftOff+6), hasContent and 0 or 1, hasContent and 16 or 0)
		TitleLbl.ZIndex                = Zindex + 3
		table.insert(Compkiller.Elements.SwitchColor, {Element=TitleLbl, Property="TextColor3"});

		if hasContent then
			ContentLbl.Name              = Compkiller:_RandomString()
			ContentLbl.Parent            = BG
			ContentLbl.BackgroundTransparency = 1
			ContentLbl.Font              = Enum.Font.GothamMedium
			ContentLbl.Text              = Config.Content
			ContentLbl.TextColor3        = Compkiller.Colors.SwitchColor
			ContentLbl.TextSize          = 11
			ContentLbl.TextTransparency  = 0.35
			ContentLbl.TextXAlignment    = Enum.TextXAlignment.Left
			ContentLbl.TextWrapped       = true
			ContentLbl.Position          = UDim2.new(0, leftOff, 0, 22)
			ContentLbl.Size              = UDim2.new(1, -(leftOff+6), 0, 16)
			ContentLbl.ZIndex            = Zindex + 3
			table.insert(Compkiller.Elements.SwitchColor, {Element=ContentLbl, Property="TextColor3"});
		end;

		BlockLine.Name                 = Compkiller:_RandomString()
		BlockLine.Parent               = LabelFrame
		BlockLine.AnchorPoint          = Vector2.new(0.5, 1)
		BlockLine.BackgroundColor3     = Compkiller.Colors.LineColor
		BlockLine.BackgroundTransparency = 0.5
		BlockLine.BorderSizePixel      = 0
		...
		Signal:Connect(function(bool)
			Compkiller:_Animation(BG,       Tween, {BackgroundTransparency = bool and 0.88 or 1});
			Compkiller:_Animation(UIStrokeBG, Tween, {Transparency = bool and 0.6 or 1});
			Compkiller:_Animation(TitleLbl, Tween, {TextTransparency = bool and 0.05 or 1});
			Compkiller:_Animation(BlockLine,Tween, {BackgroundTransparency = bool and 0.5 or 1});
			if hasContent then
				Compkiller:_Animation(ContentLbl, Tween, {TextTransparency = bool and 0.35 or 1});
			end;
		end);

		local LA = {};
		LA.Flag = Config.Flag;
		function LA:SetText(t) TitleLbl.Text = t; end;
		function LA:GetText() return TitleLbl.Text; end;
		function LA:SetContent(t) if hasContent then ContentLbl.Text = t; end; end;
		function LA:GetValue() return TitleLbl.Text; end;
		function LA:SetValue(t) TitleLbl.Text = t; end;

		if Config.Flag then Compkiller.Flags[Config.Flag] = LA; end;
		return LA;
	end;
	-- ── END AddLabel ──────────────────────────────────────────────────────────
	...
	--- @param Config { Name:string, Options:{string}, Default:string?, Flag:string?, Callback:fn }
	function Args:AddRadioGroup(Config)
		Config = Compkiller.__CONFIG(Config or {}, {
			Name     = "Radio",
		...
		if Compkiller:_IsMobile() then Compkiller:_AddDragBlacklist(GroupFrame); end;

		GroupFrame.Name              = Compkiller:_RandomString()
		GroupFrame.Parent            = Parent
		GroupFrame.BackgroundTransparency = 1
		GroupFrame.BorderSizePixel   = 0
		GroupFrame.Size              = UDim2.new(1,-1,0, 25 + #Config.Options * 26)
		GroupFrame.ZIndex            = Zindex + 1

		HeaderLbl.Name               = Compkiller:_RandomString()
		HeaderLbl.Parent             = GroupFrame
		HeaderLbl.BackgroundTransparency = 1
		HeaderLbl.Position           = UDim2.new(0,12,0,1)
		HeaderLbl.Size               = UDim2.new(1,-20,0,22)
		HeaderLbl.ZIndex             = Zindex + 2
		HeaderLbl.Font               = Enum.Font.GothamMedium
		HeaderLbl.Text               = Config.Name
		HeaderLbl.TextColor3         = Compkiller.Colors.SwitchColor
		HeaderLbl.TextSize           = 14
		HeaderLbl.TextTransparency   = 0.1
		HeaderLbl.TextXAlignment     = Enum.TextXAlignment.Left
		table.insert(Compkiller.Elements.SwitchColor, {Element=HeaderLbl, Property="TextColor3"});

		BlockLine.Name               = Compkiller:_RandomString()
		BlockLine.Parent             = GroupFrame
		BlockLine.AnchorPoint        = Vector2.new(0.5,1)
		BlockLine.BackgroundColor3   = Compkiller.Colors.LineColor
		BlockLine.BackgroundTransparency = 0.5
		BlockLine.BorderSizePixel    = 0
		...
		ItemList.Name                = Compkiller:_RandomString()
		ItemList.Parent              = GroupFrame
		ItemList.BackgroundTransparency = 1
		ItemList.BorderSizePixel     = 0
		ItemList.Position            = UDim2.new(0,0,0,24)
		ItemList.Size                = UDim2.new(1,0,1,-24)
		ItemList.ZIndex              = Zindex + 2
		...
		local function selectOption(val)
			...
			for _, btn in next , buttons do
				local isSelected = btn.Value == val;
				Compkiller:_Animation(btn.Dot,   TweenInfo.new(0.2), {
					BackgroundTransparency = isSelected and 0 or 1,
					Size = isSelected and UDim2.new(0.55,0,0.55,0) or UDim2.new(0.35,0,0.35,0)
				});
				Compkiller:_Animation(btn.Ring,  TweenInfo.new(0.2), {
					BackgroundColor3 = isSelected and Compkiller.Colors.Toggle or Compkiller.Colors.DropColor
				});
				Compkiller:_Animation(btn.Label, TweenInfo.new(0.2), {
					TextTransparency = isSelected and 0 or 0.4
				});
			end;
			pcall(function() Config.Callback(val) end);
		end;

		for _, opt in next , Config.Options do
			local Row    = Instance.new("Frame")
			local Ring   = Instance.new("Frame")
			local UCring = Instance.new("UICorner")
			local USring = Instance.new("UIStroke")
			local Dot    = Instance.new("Frame")
			local UCdot  = Instance.new("UICorner")
			...
			Ring.Name                 = Compkiller:_RandomString()
			Ring.Parent               = Row
			Ring.AnchorPoint          = Vector2.new(0,0.5)
			Ring.BackgroundColor3     = Compkiller.Colors.DropColor
			Ring.BorderSizePixel      = 0
			Ring.Position             = UDim2.new(0,14,0.5,0)
			Ring.Size                 = UDim2.new(0,16,0,16)
			Ring.ZIndex               = Zindex + 4
			UCring.CornerRadius       = UDim.new(1,0); UCring.Parent = Ring;
			USring.Color              = Compkiller.Colors.HighStrokeColor
			USring.Parent             = Ring;
			table.insert(Compkiller.Elements.DropColor,       {Element=Ring,  Property="BackgroundColor3"});
			table.insert(Compkiller.Elements.HighStrokeColor, {Element=USring,Property="Color"});

			Dot.Name                  = Compkiller:_RandomString()
			Dot.Parent                = Ring
			Dot.AnchorPoint           = Vector2.new(0.5,0.5)
			Dot.BackgroundColor3      = Compkiller.Colors.Toggle
			Dot.BackgroundTransparency = 1
			Dot.BorderSizePixel       = 0
			Dot.Position              = UDim2.new(0.5,0,0.5,0)
			Dot.Size                  = UDim2.new(0.35,0,0.35,0)
			Dot.SizeConstraint        = Enum.SizeConstraint.RelativeYY
			Dot.ZIndex                = Zindex + 5
			UCdot.CornerRadius        = UDim.new(1,0); UCdot.Parent = Dot;
			table.insert(Compkiller.Elements.Highlight, {Element=Dot, Property="BackgroundColor3"});

			Lbl.Name                  = Compkiller:_RandomString()
			Lbl.Parent                = Row
			Lbl.BackgroundTransparency = 1
			Lbl.Font                  = Enum.Font.GothamMedium
			Lbl.Text                  = tostring(opt)
			Lbl.TextColor3            = Compkiller.Colors.SwitchColor
			Lbl.TextSize              = 13
			...
			Lbl.ZIndex                = Zindex + 4
			table.insert(Compkiller.Elements.SwitchColor, {Element=Lbl, Property="TextColor3"});

			local btnRef = {Value=opt, Dot=Dot, Ring=Ring, Label=Lbl};
			table.insert(buttons, btnRef);

			Compkiller:_Input(Row, function() selectOption(opt); end);
		end;

		-- initial selection
		selectOption(Config.Default);

		Signal:Connect(function(bool)
			Compkiller:_Animation(HeaderLbl, Tween, {TextTransparency = bool and 0.1 or 1});
			Compkiller:_Animation(BlockLine, Tween, {BackgroundTransparency = bool and 0.5 or 1});
			for _, btn in next , buttons do
				Compkiller:_Animation(btn.Label, Tween, {TextTransparency = bool and (btn.Value==currentValue and 0 or 0.4) or 1});
			end;
		end);

		local RA = {};
		RA.Flag = Config.Flag;
		function RA:GetValue() return currentValue; end;
		function RA:SetValue(val)
			if table.find(Config.Options, val) then
				selectOption(val);
			end;
		end;
		function RA:SetText(str) HeaderLbl.Text = str or Config.Name; end;
		function RA:GetText() return HeaderLbl.Text; end;

		if Config.Flag then Compkiller.Flags[Config.Flag] = RA; end;
		return RA;
	end;
	-- ── END AddRadioGroup ─────────────────────────────────────────────────────
...
function Compkiller:SetTheme(name)
	-- ── CK ENHANCED: accept custom Color3 table ──────────────────────────────
	if typeof(name) == 'table' then
		for k,v in next , name do
			if Compkiller.Colors[k] ~= nil then
	...
	elseif name == "Purple" then
		Compkiller.Colors = {
			Highlight       = Color3.fromRGB(180, 90, 255),
			Toggle          = Color3.fromRGB(155, 70, 230),
			Risky           = Color3.fromRGB(255, 235, 59),
			BGDBColor       = Color3.fromRGB(20, 18, 28),
			BlockColor      = Color3.fromRGB(28, 24, 40),
			StrokeColor     = Color3.fromRGB(50, 40, 65),
			SwitchColor     = Color3.fromRGB(240, 230, 255),
			DropColor       = Color3.fromRGB(36, 30, 52),
			MouseEnter      = Color3.fromRGB(65, 50, 90),
			BlockBackground = Color3.fromRGB(45, 38, 62),
			LineColor       = Color3.fromRGB(80, 65, 100),
			HighStrokeColor = Color3.fromRGB(70, 58, 90),
		};
	elseif name == "Rose" then
		Compkiller.Colors = {
			Highlight       = Color3.fromRGB(255, 80, 120),
			Toggle          = Color3.fromRGB(230, 60, 100),
			Risky           = Color3.fromRGB(255, 240, 50),
			BGDBColor       = Color3.fromRGB(25, 18, 20),
			BlockColor      = Color3.fromRGB(36, 25, 28),
			StrokeColor     = Color3.fromRGB(55, 38, 43),
			SwitchColor     = Color3.fromRGB(255, 235, 240),
			DropColor       = Color3.fromRGB(42, 28, 32),
			MouseEnter      = Color3.fromRGB(75, 50, 55),
			BlockBackground = Color3.fromRGB(52, 38, 42),
			LineColor       = Color3.fromRGB(90, 65, 70),
			HighStrokeColor = Color3.fromRGB(75, 56, 62),
		};
	elseif name == "Emerald" then
		Compkiller.Colors = {
			Highlight       = Color3.fromRGB(46, 213, 115),
			Toggle          = Color3.fromRGB(35, 185, 95),
			Risky           = Color3.fromRGB(255, 230, 40),
			BGDBColor       = Color3.fromRGB(17, 24, 20),
			BlockColor      = Color3.fromRGB(22, 32, 26),
			StrokeColor     = Color3.fromRGB(32, 45, 37),
			SwitchColor     = Color3.fromRGB(225, 255, 235),
			DropColor       = Color3.fromRGB(27, 38, 32),
			MouseEnter      = Color3.fromRGB(45, 65, 52),
			BlockBackground = Color3.fromRGB(35, 50, 42),
			LineColor       = Color3.fromRGB(55, 80, 65),
			HighStrokeColor = Color3.fromRGB(48, 65, 55),
		};
	end;
...
-- ── CK v3: SetDPI ────────────────────────────────────────────────────────────
--- Scale the entire UI. 1.0 = 100%, 1.25 = 125%, etc.
--- @param scale number  (0.5 – 2.0 recommended)
function Compkiller:SetDPI(scale : number)
	scale = math.clamp(scale or 1, 0.25, 3);
	Compkiller._DPI = scale;

	for _, win in next , Compkiller.Windows do
		pcall(function()
			local sc = win:FindFirstChildWhichIsA('UIScale');
			if not sc then
...
--- Returns the current DPI scale (default 1).
function Compkiller:GetDPI() : number
	return Compkiller._DPI or 1;
end;

--- Returns a list of all registered theme names.
function Compkiller:GetThemeList() : {string}
	return {
...
--- Show/hide a specific window by reference. Pass the WindowArgs object.
--- @param WindowArgs table
--- @param visible boolean
function Compkiller:SetWindowVisible(WindowRef, visible : boolean)
	if not WindowRef then return end;
	pcall(function()
		-- WindowRef is a ScreenGui (stored in Compkiller.Windows)
		-- try both ScreenGui and the MainFrame approach
		for _, win in next , Compkiller.Windows do
			if win == WindowRef then
				win.Enabled = visible;
				return;
			end;
		end;
		-- fallback: treat as direct reference
		if WindowRef.Enabled ~= nil then
			WindowRef.Enabled = visible;
		end;
	end);
end;
-- ─────────────────────────────────────────────────────────────────────────────

-- ── CK v3: Global Rich Tooltip Manager ───────────────────────────────────────
-- Usage: Compkiller:Tooltip(Frame, { Title="…", Content="…", Icon="…" })
...
do
	...
	local _TT_TWEEN_IN   = TweenInfo.new(0.18, Enum.EasingStyle.Quint);
	local _TT_TWEEN_OUT  = TweenInfo.new(0.12, Enum.EasingStyle.Quint);
	local _TT_ACTIVE     = 0; -- counter to handle overlapping

	local function _EnsureTooltipGui()
		if TooltipGui and TooltipGui.Parent then return; end;

		TooltipGui = Instance.new("ScreenGui");
		TooltipGui.Name = Compkiller:_RandomString();
		TooltipGui.ResetOnSpawn = false;
		TooltipGui.ZIndexBehavior = Enum.ZIndexBehavior.Global;
		TooltipGui.Parent = CoreGui;

		TooltipFrame = Instance.new("Frame");
		TooltipFrame.Name = Compkiller:_RandomString();
		TooltipFrame.Parent = TooltipGui;
		TooltipFrame.BackgroundColor3 = Compkiller.Colors.BlockColor;
		TooltipFrame.BackgroundTransparency = 1;
		TooltipFrame.BorderSizePixel = 0;
		TooltipFrame.Size = UDim2.new(0, 220, 0, 40);
		TooltipFrame.ZIndex = 9999;
		TooltipFrame.ClipsDescendants = false;

		local uc = Instance.new("UICorner"); uc.CornerRadius = UDim.new(0,6); uc.Parent = TooltipFrame;
		TooltipStroke = Instance.new("UIStroke");
		TooltipStroke.Color = Compkiller.Colors.HighStrokeColor;
		TooltipStroke.Transparency = 1;
		TooltipStroke.Parent = TooltipFrame;

		table.insert(Compkiller.Elements.BlockColor,  {Element=TooltipFrame,  Property="BackgroundColor3"});
		table.insert(Compkiller.Elements.HighStrokeColor, {Element=TooltipStroke, Property="Color"});

		TooltipIcon = Instance.new("ImageLabel");
		TooltipIcon.Name = Compkiller:_RandomString();
		TooltipIcon.Parent = TooltipFrame;
		TooltipIcon.BackgroundTransparency = 1;
		TooltipIcon.Position = UDim2.new(0, 8, 0.5, 0);
		TooltipIcon.AnchorPoint = Vector2.new(0, 0.5);
		TooltipIcon.Size = UDim2.new(0, 16, 0, 16);
		TooltipIcon.ZIndex = 10000;
		TooltipIcon.ImageColor3 = Compkiller.Colors.Highlight;
		table.insert(Compkiller.Elements.Highlight, {Element=TooltipIcon, Property="ImageColor3"});

		TooltipTitle = Instance.new("TextLabel");
		TooltipTitle.Name = Compkiller:_RandomString();
		TooltipTitle.Parent = TooltipFrame;
		TooltipTitle.BackgroundTransparency = 1;
		TooltipTitle.Font = Enum.Font.GothamBold;
		TooltipTitle.TextColor3 = Compkiller.Colors.SwitchColor;
		TooltipTitle.TextSize = 13;
		TooltipTitle.TextXAlignment = Enum.TextXAlignment.Left;
		TooltipTitle.ZIndex = 10000;
		table.insert(Compkiller.Elements.SwitchColor, {Element=TooltipTitle, Property="TextColor3"});

		TooltipBody = Instance.new("TextLabel");
		TooltipBody.Name = Compkiller:_RandomString();
		TooltipBody.Parent = TooltipFrame;
		TooltipBody.BackgroundTransparency = 1;
		TooltipBody.Font = Enum.Font.GothamMedium;
		TooltipBody.TextColor3 = Compkiller.Colors.SwitchColor;
		TooltipBody.TextSize = 11;
		TooltipBody.TextTransparency = 0.35;
		TooltipBody.TextXAlignment = Enum.TextXAlignment.Left;
		TooltipBody.TextWrapped = true;
		TooltipBody.ZIndex = 10000;
		table.insert(Compkiller.Elements.SwitchColor, {Element=TooltipBody, Property="TextColor3"});
	end;

	local function _LayoutTooltip(cfg)
		_EnsureTooltipGui();
		...
		TooltipIcon.Visible = hasIcon;
		if hasIcon then
			TooltipIcon.Image = Compkiller:_GetIcon(cfg.Icon) or cfg.Icon;
		end;

		local iconOff = hasIcon and 30 or 8;

		TooltipTitle.Visible = hasTitle;
		TooltipTitle.Text    = cfg.Title or "";
		TooltipBody.Visible  = hasContent;
		TooltipBody.Text     = cfg.Content or "";

		if hasTitle and hasContent then
			TooltipTitle.Size     = UDim2.new(1, -(iconOff+8), 0, 16);
			TooltipTitle.Position = UDim2.new(0, iconOff, 0, 6);
			TooltipBody.Size      = UDim2.new(1, -(iconOff+8), 0, 20);
			TooltipBody.Position  = UDim2.new(0, iconOff, 0, 22);
			TooltipFrame.Size     = UDim2.new(0, 220, 0, 48);
		elseif hasTitle then
			TooltipTitle.Size     = UDim2.new(1, -(iconOff+8), 1, -8);
			TooltipTitle.Position = UDim2.new(0, iconOff, 0, 4);
			TooltipFrame.Size     = UDim2.new(0, 220, 0, 30);
		else
			TooltipBody.Size      = UDim2.new(1, -(iconOff+8), 1, -8);
			TooltipBody.Position  = UDim2.new(0, iconOff, 0, 4);
			TooltipFrame.Size     = UDim2.new(0, 220, 0, 30);
		end;

		-- auto-width based on text
		local ts = TextService:GetTextSize(TooltipTitle.Text, TooltipTitle.TextSize, TooltipTitle.Font, Vector2.new(math.huge, math.huge));
		local bs = TextService:GetTextSize(TooltipBody.Text,  TooltipBody.TextSize,  TooltipBody.Font,  Vector2.new(math.huge, math.huge));
		local w  = math.max(ts.X, bs.X) + iconOff + 16;
		w = math.clamp(w, 80, 280);
		TooltipFrame.Size = UDim2.new(0, w, 0, TooltipFrame.Size.Y.Offset);
	end;
	...
	local function _PositionTooltip()
		...
		local ox   = mpos.X + 12;
		local oy   = mpos.Y - size.Y / 2;
		if ox + size.X > vp.X - 4 then ox = mpos.X - size.X - 8; end;
		if oy < 4 then oy = 4; end;
		if oy + size.Y > vp.Y - 4 then oy = vp.Y - size.Y - 4; end;
		TooltipFrame.Position = UDim2.fromOffset(math.floor(ox), math.floor(oy));
	end;

	--- Attach a rich tooltip to any GuiObject.
	--- @param Target GuiObject
	--- @param cfg { Title:string?, Content:string?, Icon:string? }
	--- @return disconnect: () -> ()
	function Compkiller:Tooltip(Target, cfg)
		cfg = Compkiller.__CONFIG(cfg or {}, {Title="", Content="", Icon=""});
		...
		conn1 = Target.MouseEnter:Connect(function()
			...
			_LayoutTooltip(cfg);
			_PositionTooltip();
			Compkiller:_Animation(TooltipFrame,  _TT_TWEEN_IN, {BackgroundTransparency = 0.05});
			Compkiller:_Animation(TooltipStroke, _TT_TWEEN_IN, {Transparency = 0});
			if TooltipTitle.Visible then
				Compkiller:_Animation(TooltipTitle, _TT_TWEEN_IN, {TextTransparency = 0});
			end;
			if TooltipBody.Visible then
				Compkiller:_Animation(TooltipBody, _TT_TWEEN_IN, {TextTransparency = 0.35});
			end;
		end);

		conn2 = Target.MouseLeave:Connect(function()
			_TT_ACTIVE = math.max(0, _TT_ACTIVE - 1);
			if _TT_ACTIVE == 0 then
				Compkiller:_Animation(TooltipFrame,  _TT_TWEEN_OUT, {BackgroundTransparency = 1});
				Compkiller:_Animation(TooltipStroke, _TT_TWEEN_OUT, {Transparency = 1});
			end;
		end);

		conn3 = UserInputService.InputChanged:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseMovement and _TT_ACTIVE > 0 then
				_PositionTooltip();
			end;
		end);

		table.insert(Compkiller._CK_Connections, conn1);
		table.insert(Compkiller._CK_Connections, conn2);
		table.insert(Compkiller._CK_Connections, conn3);

		return function()
			pcall(function() conn1:Disconnect(); conn2:Disconnect(); conn3:Disconnect(); end);
		end;
	end;
end;
-- ── END Tooltip Manager ───────────────────────────────────────────────────────
