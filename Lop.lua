--[[
    MODERN UI LIBRARY V5 - PLATINUM EDITION
    Author: Gemini (Updated for User)
    
    [CHANGELOG V5]
    1. Mobile Fix: Sử dụng Enum.AutomaticSize cho tất cả ScrollingFrame -> Vuốt mượt mà trên Mobile.
    2. Settings Integration: Nút bánh răng ở Profile giờ sẽ mở trang Settings riêng biệt.
    3. New Elements: 
       - ChipSet (Horizontal Selection)
       - ColorPicker (RGB Visual)
    4. QoL:
       - Watermark (FPS/Ping)
       - Key System Template
]]
...
local Library = {
    ...
    CurrentTheme = "Dark",
    MainFrameRef = nil,
    SettingsPageRef = nil, -- Reference to hidden settings page
}
...
function Library:SetTheme(ThemeName)
    if Library.Themes[ThemeName] then Library.Theme = Library.Themes[ThemeName]; Library.CurrentTheme = ThemeName end
end
...
function Library:Watermark(Title)
    ...
    Create("UIStroke", {Parent = WFrame, Color = Library.Theme.Gradient1, Thickness = 1})
    ...
    task.spawn(function()
        while WFrame.Parent do
            ...
            Text.Text = (Title or "Lop UI") .. " | " .. os.date("%H:%M") .. " | FPS: " .. fps .. " | Ping: " .. ping .. "ms"
            task.wait(1)
        end
    end)
end
...
local function RegisterContainerFunctions(Container, PageInstance)
    ...
    function Funcs:AddToggle(Config)
        ...
        function ToggleObj:Extend()
            ...
            local ExtContent = Create("ScrollingFrame", {Parent = ExtensionFrame, BackgroundTransparency = 1, Position = UDim2.new(0,0,0,35), Size = UDim2.new(1,0,1,-35), CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarThickness = 2})
    ...
    function Funcs:AddSlider(Config)
        ...
        local ValLbl = Create("TextLabel", {Parent = SFrame, BackgroundTransparency = 1, Position = UDim2.new(1, -50, 0, 5), Size = UDim2.new(0, 40, 0, 20), Text = tostring(Def), Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextSize = 12})
        ...
        local function Update(i)
            ...
            Fill.Size = UDim2.new(s, 0, 1, 0); ValLbl.Text = tostring(res)
    ...
    --// NEW: CHIPSET (Horizontal Selection)
    function Funcs:AddChipSet(Config)
        -- Config: Text, Items = {}, Callback(val)
        ...
        local Scroll = Create("ScrollingFrame", {
            Parent = CFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0, 25), Size = UDim2.new(1, -10, 0, 30),
            CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.X, ScrollBarThickness = 0, ScrollingDirection = Enum.ScrollingDirection.X
        })
        ...
        for _, item in ipairs(Config.Items) do
            local Chip = Create("TextButton", {
                Parent = Scroll, BackgroundColor3 = Library.Theme.Section, Size = UDim2.new(0, 0, 1, 0), AutomaticSize = Enum.AutomaticSize.X,
                Text = "  " .. item .. "  ", Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextSize = 12
            })
            Create("UICorner", {Parent = Chip, CornerRadius = UDim.new(0, 15)})
            Chip.MouseButton1Click:Connect(function()
                if Current then Current.BackgroundColor3 = Library.Theme.Section; Current.TextColor3 = Library.Theme.SubText end
                Current = Chip
    ...
    function Funcs:AddColorPicker(Config)
        ...
        local PickerFrame = Create("Frame", {
            Parent = CPFrame, BackgroundColor3 = Library.Theme.Element, Position = UDim2.new(0,0,1,5), Size = UDim2.new(1,0,0,100), Visible = false, ZIndex = 10
        }); Create("UICorner", {Parent = PickerFrame, CornerRadius = UDim.new(0,6)})
        
        -- Simply Inputs for R, G, B for stability in single file
        local function AddInput(Name, Pos, Get, Set)
            ...
            local Box = Create("TextBox", {Parent = PickerFrame, Position=UDim2.new(0, 35, 0, Pos), Size=UDim2.new(0, 50, 0, 20), BackgroundColor3=Library.Theme.Section, Text=tostring(Get), TextColor3=Library.Theme.Text})
            Create("UICorner", {Parent=Box, CornerRadius=UDim.new(0,4)})
            Box.FocusLost:Connect(function() 
                local n = tonumber(Box.Text); if n then Set(math.clamp(n,0,255)) end 
                local c = Preview.BackgroundColor3
                Preview.BackgroundColor3 = Color3.fromRGB(c.R*255, c.G*255, c.B*255) -- Update visual handled by setters
        ...
        Preview.MouseButton1Click:Connect(function()
            ...
            CPFrame.Size = IsOpen and UDim2.new(1,0,0,145) or UDim2.new(1,0,0,40)
            local c = Preview.BackgroundColor3
            R, G, B = math.floor(c.R*255), math.floor(c.G*255), math.floor(c.B*255)
            BoxR.Text=R; BoxG.Text=G; BoxB.Text=B
        end)
    end
        ...
        local function Refresh(txt)
            ...
            for _, item in pairs(Config.Items) do
                if txt == "" or item:lower():find(txt:lower()) then
                    local B = Create("TextButton", {Parent = List, BackgroundColor3 = Library.Theme.Section, Size = UDim2.new(1, -10, 0, 25), Text = item, Font = Enum.Font.Gotham, TextColor3 = Library.Theme.SubText, TextSize = 12}); Create("UICorner", {Parent = B, CornerRadius = UDim.new(0, 4)})
                    B.MouseButton1Click:Connect(function() IsOpen = false; TweenService:Create(DFrame, TweenInfo.new(0.3), {Size = UDim2.new(1, 0, 0, 40)}):Play(); TweenService:Create(Arrow, TweenInfo.new(0.3), {Rotation = 0}):Play(); Label.Text = Config.Text..": "..item; if Config.Callback then Config.Callback(item) end end)
                end
            end
        end
...
function Library:CreateWindow(Config)
    ...
    local ScreenGui = Create("ScreenGui", {Name = "ModernUI_V5", Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or CoreGui, ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling})
    ...
    function Window:CreateTab(Name)
        local Tab = {Elements = {}}
        local Page = Create("ScrollingFrame", {Name = Name, Parent = PageHolder, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Visible = false, CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarThickness = 4, ScrollingDirection = Enum.ScrollingDirection.Y})
        ...
        Tab.Activate = Activate; RegisterContainerFunctions(Tab, Page); return Tab
    end
    
    -- Internal Settings Page (Hidden)
    local SettingsPage = Create("ScrollingFrame", {Name = "Settings", Parent = PageHolder, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Visible = false, CanvasSize = UDim2.new(0,0,0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollBarThickness = 4})
    ...
    SettingsHandler:AddParagraph({Title = "Library Settings", Content = "Configure themes and UI options."})
    SettingsHandler:AddChipSet({Text = "Themes", Items = {"Dark", "PastelPurple", "Sky", "Garden"}, Callback = function(v) Library:SetTheme(v); Library:Notify({Title = "Theme Updated", Content = v, Duration = 2}) end})
