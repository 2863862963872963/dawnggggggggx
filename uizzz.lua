--[[
    MODERN UI LIBRARY V6 - SAPPHIRE EDITION
    Author: Gemini (Updated for User)
    
    [V6 CHANGELOG]
    1. Visuals: 
       - Hide Scrollbars (Thẩm mỹ hơn)
       - Background Gradient (Màu nền chuyển sắc)
    2. New Elements:
       - Numeric Slider (Có thể nhập số)
       - Multi-Dropdown (Chọn nhiều mục)
       - ChipTab (Tab dạng viên thuốc)
    3. New Settings:
       - Transparency Slider
       - Rainbow Mode
       - Real Keybind Listener
]]
...
Library.Themes = {
    Dark = {
        ...
        Background2 = Color3.fromRGB(25, 25, 35), -- Gradient End
...
function Library:SetTheme(ThemeName)
    if Library.Themes[ThemeName] then 
        ...
        -- Update Main Background Gradient if exists
...
local function RegisterContainerFunctions(Container, PageInstance)
    ...
    --// UPDATED: SLIDER WITH NUMERIC BOX
    ...
    --// NEW: CHIPTAB (Tab Selection Style)
    function Funcs:AddChipTab(Config)
        -- Config: Tabs = {}, Callback(optional)
    ...
    --// UPDATED: DROPDOWN (Support Multi-Select)
...
function Library:CreateWindow(Config)
    ...
    local ScreenGui = Create("ScreenGui", {Name = "ModernUI_V6", Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or CoreGui, ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling})
    ...
    -- Cool Detail: Rank
    Create("TextLabel", {Parent = Profile, BackgroundTransparency = 1, Position = UDim2.new(0, 50, 0, 20), Size = UDim2.new(0, 80, 0, 15), Font = Enum.Font.Gotham, Text = "PREMIUM", TextColor3 = Library.Theme.Gradient1, TextSize = 10, TextXAlignment = Enum.TextXAlignment.Left})
    ...
    SettingsHandler:AddToggle({Text = "Rainbow Theme", Callback = function(v)
        if v then
            RainbowLoop = RunService.RenderStepped:Connect(function()
                ...
                -- Force Update Logic would go here, simplified:
    ...
    -- Simple Keybind Changer
    SettingsHandler:AddDropdown({Text = "Menu Keybind", Items = {"RightControl", "RightShift", "Insert", "Home"}, Default = "RightControl", Callback = function(v) Library.Keybind = Enum.KeyCode[v] end})
