...
--[[
	SetDPI(scale: number)
	Scales the entire UI by a given factor using UIScale.
	Call this BEFORE Compkiller.new() to apply globally, or pass a ScreenGui.
	  scale = 1.0  →  100% (default)
	  scale = 1.25 →  125%
	  scale = 0.85 →  85%
	The scale is clamped between 0.5 and 2.0 for stability.
]]
function Compkiller:SetDPI(scale: number, targetGui: ScreenGui?)
	scale = math.clamp(scale or 1, 0.5, 2.0);
	Compkiller.DPIScale = scale;

	local function applyToGui(gui: ScreenGui)
		...
		if scale == 1.0 then return end; -- no-op at 100%
	...
	if targetGui then
		applyToGui(targetGui);
	else
		-- Apply to all registered windows
		for _, gui in next, Compkiller.Windows do
...
function Compkiller.new(Config : Window)
	...
	-- Apply active DPI scale if one has been set before this window was created
	if Compkiller.DPIScale and Compkiller.DPIScale ~= 1 then
		Compkiller:SetDPI(Compkiller.DPIScale, CompKiller);
	end;
	...
	MainFrame.Size = UDim2.new(
		...
		Config.Scale.X.Offset - 18,
		Config.Scale.Y.Scale,
		Config.Scale.Y.Offset - 18
	); -- start slightly smaller for entry pop
	MainFrame.BackgroundTransparency = 0.6; -- start semi-transparent
	MainFrame.ZIndex = 4
	...
	-- Smooth entry: expand + fade in with Quint overshoot feel
	Compkiller:_Animation(MainFrame, TweenInfo.new(0.65, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
		Size = Config.Scale,
		BackgroundTransparency = 0
	});
	...
	WindowOpen:Connect(function(v)
		...
		-- Smooth Quint-eased open / close with staggered content fade
		local QuintOut  = TweenInfo.new(0.45, Enum.EasingStyle.Quint, Enum.EasingDirection.Out);
		local QuintIn   = TweenInfo.new(0.30, Enum.EasingStyle.Quint, Enum.EasingDirection.In);
		local FadeSlow  = TweenInfo.new(0.40, Enum.EasingStyle.Quint, Enum.EasingDirection.Out);
		local FadeFast  = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out);
		local SlideTabs = TweenInfo.new(0.50, Enum.EasingStyle.Quint, Enum.EasingDirection.Out);
		local LineAnim  = TweenInfo.new(0.45, Enum.EasingStyle.Quint, Enum.EasingDirection.Out);

		if v then
			-- Frame expands in from slightly smaller size
			Compkiller:_Animation(MainFrame, QuintOut, {
				Size = Config.Scale,
				BackgroundTransparency = 0
			})

			-- Tab panel glides in
			Compkiller:_Animation(TabFrame, FadeSlow, {
				BackgroundTransparency = TabFrameBaseTrans
			})

			-- Slide tab buttons back into view
			Compkiller:_Animation(TabButtonScrollingFrame, SlideTabs, {
				Position = UDim2.new(0.5, 0, 0.5, 0)
			})

			-- Separator line expands
			Compkiller:_Animation(LineFrame1, LineAnim, {
				BackgroundTransparency = 0,
			...
			task.delay(0.05, function()
				Compkiller:_Animation(CompLogo, FadeSlow, {
					ImageTransparency = 0
				})
				Compkiller:_Animation(WindowLabel, FadeSlow, {
					TextTransparency = 0
				})
			end)

			task.delay(0.10, function()
				Compkiller:_Animation(UserProfile, FadeSlow, {
					ImageTransparency = 0
				})
				Compkiller:_Animation(UserText, FadeSlow, {
					TextTransparency = 0
				})
				Compkiller:_Animation(ExpireText, FadeSlow, {
					TextTransparency = 0.5
				})
			end)
		else
			-- Content fades out first, then frame collapses
			Compkiller:_Animation(CompLogo, FadeFast, {
				ImageTransparency = 1
			})
			Compkiller:_Animation(WindowLabel, FadeFast, {
				TextTransparency = 1
			})
			Compkiller:_Animation(UserProfile, FadeFast, {
				ImageTransparency = 1
			})
			Compkiller:_Animation(UserText, FadeFast, {
				TextTransparency = 1
			})
			Compkiller:_Animation(ExpireText, FadeFast, {
				TextTransparency = 1
			})
			...
			-- Line collapses quickly
			Compkiller:_Animation(LineFrame1, FadeFast, {
				BackgroundTransparency = 1,
				Size = UDim2.new(0, 1, 1, 0)
			})

			-- Frame shrinks and fades with slight delay so content clears first
			task.delay(0.06, function()
				Compkiller:_Animation(MainFrame, QuintIn, {
					Size = UDim2.new(
						math.max(Config.Scale.X.Scale - 0.03, 0),
						Config.Scale.X.Offset - 8,
						math.max(Config.Scale.Y.Scale - 0.03, 0),
						Config.Scale.Y.Offset - 8
					),
					BackgroundTransparency = 1
				})
				Compkiller:_Animation(TabFrame, QuintIn, {
					BackgroundTransparency = 1
				})
			end)
...
function Compkiller:Loader(IconId,Duration)
	...
	Compkiller:_Animation(Loader,TweenInfo.new(0.65,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{
		...
		new = function(Notify: Notify) : NotifyPayback
			...
			local UpdateText = function()
				...
				if BlockFrame:GetAttribute('Already') then
					Compkiller:_Animation(BlockFrame,TweenInfo.new(0.35,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{
					...
					Compkiller:_Animation(BlockFrame,TweenInfo.new(0.40,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{
						Size = UDim2.new(0,MainX + 55,0,MainY + 35)
					});
				end;
			end;
			...
			local Close = function()
				Compkiller:_Animation(NotifyFrame,TweenInfo.new(0.55,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{
					Position = UDim2.new(1,220,0,0)
				});

				task.wait(0.28);

				Compkiller:_Animation(BlockFrame,TweenInfo.new(0.32,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{
			...
			local Show = function()
				Compkiller:_Animation(NotifyFrame,TweenInfo.new(0.55,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{
