...
function library:window(properties)
	local cfg = {
		-- Properties
		name = properties.name or properties.Name or "(panichook11, nomercy.rip, fijihook.fun, desktopware, templehook, pulse, brick heck ui services)",
...
function library:Slider(options)
	...
	library:connection(uis.InputChanged, function(input)
		if cfg.dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
			local size_x = (input.Position.X - items["gradient_holder"].AbsolutePosition.X)
...
function library:Dropdown(options)
	...
	function cfg.set_visible(bool)
		...
		items["dropdown_holder"].Position =
			dim2(0, items.dropdown_outline.AbsolutePosition.X, 0, items.dropdown_outline.AbsolutePosition.Y + 75)
...
function library:Colorpicker(options)
	...
	function cfg.set_visible(bool)
		...
		items.colorpicker_outline.Position = dim2(
			...
			0,
			items.gear_holder.AbsolutePosition.Y + items.gear_holder.AbsoluteSize.Y + 60 - 19
		)
	...
	function cfg.update_color()
		...
		elseif dragging_alpha then
			a = 1
				- math.clamp((offset - items.alpha_slider.AbsolutePosition).X / items.alpha_slider.AbsoluteSize.X, 0, 1)
		end
...
local window = library:window({ name = "(panichook11, nomercy.rip, fijihook.fun, desktopware, templehook, pulse, brick heck ui services)" })
