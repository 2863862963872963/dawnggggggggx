...
function Utils.GetDPIScale()
	...
	if isMobile and not isTablet then
		-- Phone: Scale based on typical phone width (360-428px)
		local phoneScale = math.clamp(viewportSize.X / 1920, 0.3, 0.6)
		return phoneScale
	elseif isTablet then
		-- Tablet: Scale based on typical tablet width (768-1024px)
		local tabletScale = math.clamp(viewportSize.X / 1920, 0.5, 0.8)
		return tabletScale
	else
		-- Desktop: Normal scaling
		local desktopScale = math.clamp(viewportSize.X / 1920, 0.7, 1.5)
		return desktopScale
	end
end
...
function Window:CreateTab(config)
	...
	local dpiScale = self.DPIScale or Utils.GetDPIScale()
	local isMobile = self.IsMobile or (Utils.IsMobile() and not Utils.IsTablet())
	local navItemHeight = math.floor((isMobile and 40 or 44) * dpiScale / (isMobile and 0.4 or 1))
	
	local navItem = Instance.new("TextButton")
	navItem.Size = UDim2.new(1, 0, 0, navItemHeight)
	...
	local cornerRadius = math.floor(10 * dpiScale / (isMobile and 0.4 or 1))
	Utils.CreateCorner(navItem, cornerRadius)
	
	local indicator = Instance.new("Frame")
	indicator.Size = UDim2.new(0, math.floor(3 * dpiScale / (isMobile and 0.4 or 1)), 0, 0)
	...
	Utils.CreateCorner(indicator, UDim.new(0, math.floor(3 * dpiScale / (isMobile and 0.4 or 1))))
	...
	if isMobile then
		...
		local iconSize = math.floor(18 * dpiScale / 0.4)
		local icon = Components.CreateIcon(contentFrame, tab.Icon, iconSize, Theme.Colors.TextMain)
		icon.Position = UDim2.new(0.5, -iconSize/2, 0.5, -iconSize/2)
		tab.IconImage = icon
	else
		-- Desktop: Icon + Text
		local padding = math.floor(15 * dpiScale)
		Utils.CreatePadding(contentFrame, {Left = padding, Right = padding})
		
		local layout = Instance.new("UIListLayout")
		layout.FillDirection = Enum.FillDirection.Horizontal
		layout.VerticalAlignment = Enum.VerticalAlignment.Center
		layout.Padding = UDim.new(0, math.floor(12 * dpiScale))
		layout.Parent = contentFrame
		
		local iconSize = math.floor(18 * dpiScale)
		local icon = Components.CreateIcon(contentFrame, tab.Icon, iconSize, Theme.Colors.TextMain)
		tab.IconImage = icon
		...
		label.Text = tab.Name
		label.Font = Enum.Font.Gotham
		label.TextSize = math.floor(13 * dpiScale)
		label.TextColor3 = Theme.Colors.TextMain
		label.TextXAlignment = Enum.TextXAlignment.Left
		...
		if config.Badge then
			...
			badge.Size = UDim2.new(0, 0, 0, math.floor(18 * dpiScale))
			badge.AutomaticSize = Enum.AutomaticSize.X
			badge.BackgroundColor3 = Theme.Colors.Danger
			badge.Text = config.Badge
			badge.Font = Enum.Font.GothamBold
			badge.TextSize = math.floor(9 * dpiScale)
			badge.TextColor3 = Color3.new(1, 1, 1)
			badge.Parent = contentFrame
			
			Utils.CreateCorner(badge, math.floor(9 * dpiScale))
			Utils.CreatePadding(badge, {Left = math.floor(6 * dpiScale), Right = math.floor(6 * dpiScale)})
		end
	end
	...
	function tab:CreateSection(sectionConfig)
		...
		local dpiScale = self.Window.DPIScale or Utils.GetDPIScale()
		local isMobile = self.Window.IsMobile or (Utils.IsMobile() and not Utils.IsTablet())
		...
		Utils.CreateCorner(sectionCard, math.floor(12 * dpiScale / (isMobile and 0.4 or 1)))
		Utils.CreateStroke(sectionCard, Theme.Colors.GlassBorder, math.max(1, math.floor(1 * dpiScale / (isMobile and 0.4 or 1))), Theme.Transparency.GlassBorder)
		
		local headerHeight = math.floor((isMobile and 50 : 60) * dpiScale / (isMobile and 0.4 or 1))
		local cardHeader = Instance.new("Frame")
		cardHeader.Size = UDim2.new(1, 0, 0, headerHeight)
		cardHeader.BackgroundTransparency = 1
		cardHeader.Parent = sectionCard
		
		local headerPadding = math.floor((isMobile and 12 or 20) * dpiScale / (isMobile and 0.4 or 1))
		Utils.CreatePadding(cardHeader, headerPadding)
		
		local headerLayout = Instance.new("UIListLayout")
		headerLayout.FillDirection = Enum.FillDirection.Horizontal
		headerLayout.VerticalAlignment = Enum.VerticalAlignment.Center
		headerLayout.Padding = UDim.new(0, math.floor(10 * dpiScale / (isMobile and 0.4 or 1)))
		headerLayout.Parent = cardHeader
		
		local iconSize = math.floor((isMobile and 28 or 40) * dpiScale / (isMobile and 0.4 or 1))
		...
		Utils.CreateCorner(iconFrame, math.floor(8 * dpiScale / (isMobile and 0.4 or 1)))
		
		local iconImageSize = math.floor((isMobile and 16 or 20) * dpiScale / (isMobile and 0.4 or 1))
		local iconImage = Components.CreateIcon(iconFrame, section.Icon, iconImageSize, 
			section.IconColor == "primary" and Theme.Colors.Primary or 
			section.IconColor == "danger" and Theme.Colors.Danger or
			section.IconColor == "success" and Theme.Colors.Success or
			Theme.Colors.Warning)
		iconImage.Position = UDim2.new(0.5, -iconImageSize/2, 0.5, -iconImageSize/2)
		
		local titleLabel = Instance.new("TextLabel")
		titleLabel.Size = UDim2.new(1, -(iconSize + math.floor(10 * dpiScale / (isMobile and 0.4 or 1))), 1, 0)
		titleLabel.BackgroundTransparency = 1
		titleLabel.Text = section.Title
		titleLabel.Font = Enum.Font.GothamBold
		titleLabel.TextSize = math.floor((isMobile and 13 or 16) * dpiScale / (isMobile and 0.4 or 1))
		titleLabel.TextColor3 = Theme.Colors.TextMain
		titleLabel.TextXAlignment = Enum.TextXAlignment.Left
		titleLabel.TextScaled = isMobile and true or false
		...
		local contentPadding = math.floor((isMobile and 12 or 20) * dpiScale / (isMobile and 0.4 or 1))
		Utils.CreatePadding(contentContainer, {Top = 0, Bottom = contentPadding, Left = contentPadding, Right = contentPadding})
		
		local contentLayout = Instance.new("UIListLayout")
		contentLayout.Padding = UDim.new(0, math.floor((isMobile and 10 or 15) * dpiScale / (isMobile and 0.4 or 1)))
		...
		function section:AddToggle(config)
			...
			local elementHeight = math.floor((isMobile and 45 or 50) * dpiScale / (isMobile and 0.4 or 1))
			local elementFrame = Instance.new("Frame")
			elementFrame.Size = UDim2.new(1, 0, 0, elementHeight)
			elementFrame.BackgroundTransparency = 1
			elementFrame.LayoutOrder = #self.Elements + 1
			elementFrame.Parent = self.Container
			
			local labelContainer = Instance.new("Frame")
			labelContainer.Size = UDim2.new(1, -(math.floor(55 * dpiScale / (isMobile and 0.4 or 1))), 1, 0)
			labelContainer.BackgroundTransparency = 1
			labelContainer.Parent = elementFrame
			
			local labelLayout = Instance.new("UIListLayout")
			labelLayout.Padding = UDim.new(0, math.floor(3 * dpiScale / (isMobile and 0.4 or 1)))
			labelLayout.VerticalAlignment = Enum.VerticalAlignment.Center
			labelLayout.Parent = labelContainer
			
			local mainLabel = Instance.new("TextLabel")
			mainLabel.Size = UDim2.new(1, 0, 0, math.floor(16 * dpiScale / (isMobile and 0.4 or 1)))
			mainLabel.BackgroundTransparency = 1
			mainLabel.Text = config.Title or "Toggle"
			mainLabel.Font = Enum.Font.GothamSemibold
			mainLabel.TextSize = math.floor((isMobile and 12 or 14) * dpiScale / (isMobile and 0.4 or 1))
			mainLabel.TextColor3 = Theme.Colors.TextMain
			mainLabel.TextXAlignment = Enum.TextXAlignment.Left
			mainLabel.TextScaled = isMobile and true or false
			mainLabel.Parent = labelContainer
			
			if config.Description then
				local descLabel = Instance.new("TextLabel")
				descLabel.Size = UDim2.new(1, 0, 0, math.floor(12 * dpiScale / (isMobile and 0.4 or 1)))
				descLabel.BackgroundTransparency = 1
				descLabel.Text = config.Description
				descLabel.Font = Enum.Font.Gotham
				descLabel.TextSize = math.floor((isMobile and 10 or 12) * dpiScale / (isMobile and 0.4 or 1))
				descLabel.TextColor3 = Theme.Colors.TextMuted
				descLabel.TextXAlignment = Enum.TextXAlignment.Left
				descLabel.TextScaled = isMobile and true or false
				descLabel.Parent = labelContainer
			end
			
			local toggleWidth = math.floor(44 * dpiScale / (isMobile and 0.4 or 1))
			local toggleHeight = math.floor(24 * dpiScale / (isMobile and 0.4 or 1))
			local toggle = Components.CreateToggle(elementFrame, config.Default or false)
			toggle.Frame.Size = UDim2.new(0, toggleWidth, 0, toggleHeight)
			toggle.Frame.Position = UDim2.new(1, -toggleWidth, 0.5, -toggleHeight/2)
...
function AscentUI:CreateWindow(config)
	...
	-- Sidebar
	local dpiScale = Utils.GetDPIScale()
	local sidebarWidth = isMobile and math.floor(60 * (dpiScale / 0.4)) or 260
	...
	-- Brand section
	local brandHeight = math.floor((isMobile and 55 or 80) * dpiScale / (isMobile and 0.4 or 1))
	
	local brand = Instance.new("Frame")
	brand.Size = UDim2.new(1, 0, 0, brandHeight)
	...
	if not isMobile then
		...
		local iconSize = math.floor(24 * dpiScale / 0.4)
		local icon = Components.CreateIcon(iconContainer, "Target", iconSize, Theme.Colors.Primary)
		icon.Position = UDim2.new(0.5, -iconSize/2, 0.5, -iconSize/2)
	end
	
	-- Navigation group
	local statusBarHeight = math.floor((isMobile and 55 or 80) * dpiScale / (isMobile and 0.4 or 1))
	
	local navGroup = Instance.new("ScrollingFrame")
	navGroup.Size = UDim2.new(1, 0, 1, -(brandHeight + statusBarHeight))
	navGroup.Position = UDim2.new(0, 0, 0, brandHeight)
	navGroup.BackgroundTransparency = 1
	navGroup.BorderSizePixel = 0
	navGroup.ScrollBarThickness = math.floor(4 * dpiScale / (isMobile and 0.4 or 1))
	navGroup.ScrollBarImageColor3 = Theme.Colors.Primary
	navGroup.ScrollBarImageTransparency = 0.8
	navGroup.Parent = sidebar
	
	local navPadding = math.floor((isMobile and 4 or 10) * dpiScale / (isMobile and 0.4 or 1))
	Utils.CreatePadding(navGroup, {Top = navPadding, Bottom = navPadding, Left = navPadding, Right = navPadding})
	...
	statusBar.Size = UDim2.new(1, 0, 0, statusBarHeight)
	statusBar.Position = UDim2.new(0, 0, 1, -statusBarHeight)
	...
	workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
		local newDpiScale = Utils.GetDPIScale()
		local newIsMobile = Utils.IsMobile()
		local newIsTablet = Utils.IsTablet()
		
		-- Recalculate sidebar width
		local newSidebarWidth = (newIsMobile and not newIsTablet) and math.floor(60 * (newDpiScale / 0.4)) or 260
		...
		if newIsMobile and not newIsTablet then
			...
			main.Size = UDim2.new(1, -newSidebarWidth, 1, 0)
			main.Position = UDim2.new(0, newSidebarWidth, 0, 0)
		...
		window.DPIScale = newDpiScale
		window.SidebarWidth = newSidebarWidth
		window.IsMobile = newIsMobile and not newIsTablet
